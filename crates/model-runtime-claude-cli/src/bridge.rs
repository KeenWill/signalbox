...
fn serve(catalog_path: PathBuf, ready_path: PathBuf) -> ExitCode {
    ...
    let Some(catalog) = parse_catalog(&catalog_bytes) else {
        return ExitCode::FAILURE;
    };
...
fn parse_catalog(bytes: &[u8]) -> Option<Catalog> {
    ...
    let catalog = {
        ...
        Catalog::deserialize(deserializer).ok()?
    };
    deserializer.end().ok()?;
    Some(catalog)
}
...
fn catalog_is_valid(catalog: &Catalog) -> bool {
    ...
    catalog.tools.iter().all(|tool| {
        valid_mcp_tool_name(&tool.name)
            ...
            && tool.input_schema.get().trim_start().starts_with('{')
    })
}
...
fn write_response(output: &mut impl Write, response: &impl Serialize) -> std::io::Result<()> {
