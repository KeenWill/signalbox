...
async fn execute_process<C: Clone + Send + Sync>(
    ...
    let Some(deadline) = tokio::time::Instant::now().checked_add(prepared.exchange_timeout) else {
        ...
        return pre_exchange_transport_loss(
            "Claude CLI exchange timeout cannot be represented by the runtime clock",
        );
    };
    let Some(mut stdin) = child.stdin.take() else {
        force_kill(&mut child).await;
        return pre_exchange_transport_loss("spawned Claude process has no stdin");
    };
    let Some(stdout) = child.stdout.take() else {
        force_kill(&mut child).await;
        return pre_exchange_transport_loss("spawned Claude process has no stdout");
    };
    let Some(stderr) = child.stderr.take() else {
        force_kill(&mut child).await;
        return pre_exchange_transport_loss("spawned Claude process has no stderr");
    };
    ...
    let input_error = match input_step {
        ...
        InputStep::Cancelled => {
            ...
            if leader_exited_without_reaping(child.id()) {
                Some(std::io::Error::other(
                    "request upload cancelled after the Claude CLI leader had exited",
                ))
            } else {
                interrupt_then_kill(
                    &mut child,
        ...
        InputStep::TimedOut => {
            ...
            if leader_exited_without_reaping(child.id()) {
                Some(std::io::Error::other(
                    "request upload timed out after the Claude CLI leader had exited",
                ))
            } else {
                force_kill(&mut child).await;
                abort_stderr_task(&mut stderr_task).await;
                return pre_exchange_boundary_loss(LossCause::TimedOut(TransportFacts::new(
                    "Claude CLI process exceeded its exchange timeout",
                )));
            }
        }
    };
                ...
                match child.try_wait() {
                        ...
                        if exited_before_cleanup || !was_killed_by_group_cleanup(&status) =>
                    {
                        ...
                        let stderr_detail = drain_stderr_after_cleanup(
                            &mut stderr_task,
                            "Claude stderr was unavailable at the process-cleanup deadline",
                        )
                        .await;
        ...
        None => {
            ...
            if let Err(error) = exit_ready {
                ...
                return decoder.boundary_loss(LossCause::TransportFailed(TransportFacts::new(
                    format!("could not observe Claude CLI process exit safely: {error}"),
                )));
            }
    ...
    match status {
        ...
        Ok(status) => {
            ...
            let (message, classification) = if !stderr_detail.trim().is_empty() {
                (
                    format!("Claude CLI exited with status {status}: {stderr_detail}"),
                    format!("Claude CLI exited with status {status}: {}", stderr.trim()),
                )
            } else if let Some(error) = input_error {
                let message =
                    format!("Claude CLI exited with status {status} after stdin failed: {error}");
                (message.clone(), message)
            } else {
                let message = format!("Claude CLI exited with status {status}");
                (message.clone(), message)
            };
        ...
        Err(error) => {
            ...
            decoder.boundary_loss(LossCause::TransportFailed(TransportFacts::new(format!(
                "could not wait for Claude CLI process: {error}"
            ))))
        }
    }
}

/// `CLAUDE_CONFIG_DIR` names the CLI's login store — with `HOME` supplying its
/// `$HOME/.claude` fallback — and the child interprets both after
...
/// The absolute form of a `HOME`/`CLAUDE_CONFIG_DIR` value, resolved through
...
/// child's login store at `<cwd>/.claude`; a relative value whose absolutization
...
/// proxy-userinfo rule (INV-035) — and a `HOME`/`CLAUDE_CONFIG_DIR` the parent cannot
...
fn allowlisted_environment(
    ...
    for name in CLAUDE_ENVIRONMENT_ALLOWLIST {
...
enum EnvironmentRejectionReason {
    ...
    Unverifiable,
    /// `HOME`/`CLAUDE_CONFIG_DIR` is present but cannot be resolved to an absolute
    ...
    fn diagnostic(&self) -> String {
        ...
        match self.reason {
            EnvironmentRejectionReason::EmbedsUserinfo => format!(
                "inherited `{name}` embeds URL userinfo; the Claude CLI would receive that \
            ...
            EnvironmentRejectionReason::UnresolvableCredentialHome => format!(
                "inherited `{name}` cannot be resolved to an absolute directory — it is empty, \
                 or the working directory it would resolve against is gone — so the Claude login \
...
fn prepared_environment_value(
    ...
    if matches!(name, "HOME" | "CLAUDE_CONFIG_DIR") {
...
async fn reap_exited_leader(
    ...
    let detail = drain_stderr_after_cleanup(
        stderr_task,
        "Claude stderr was unavailable after cancellation",
    )
    .await;
...
fn incomplete_upload_cause(error: &std::io::Error) -> LossCause {
    LossCause::TransportFailed(TransportFacts::new(format!(
        "Claude stdin closed before the full request upload completed: {error}"
    )))
}
...
fn stderr_result(result: Result<std::io::Result<String>, tokio::task::JoinError>) -> String {
    match result {
        Ok(Ok(stderr)) => stderr,
        Ok(Err(error)) => format!("could not read Claude stderr: {error}"),
        Err(error) => format!("Claude stderr reader failed: {error}"),
    }
}
...
fn oversize_event(limit: usize) -> std::io::Error {
    std::io::Error::new(
        ...
        format!("Claude JSONL event exceeded the {limit}-byte limit"),
    )
}
...
async fn wait_for_exit_without_reaping(process_group_id: Option<u32>) -> std::io::Result<()> {
    let raw_pid = process_group_id.ok_or_else(|| {
        std::io::Error::other("spawned Claude process has no process-group identity")
    })?;
    let pid = rustix::process::Pid::from_raw(raw_pid as i32).ok_or_else(|| {
        std::io::Error::other("spawned Claude process has an invalid process-group identity")
    })?;
    tokio::task::spawn_blocking(move || {
        ...
        status
            .map(drop)
            .ok_or_else(|| std::io::Error::other("Claude process exit was not observable"))
    })
    .await
    .map_err(|error| std::io::Error::other(format!("Claude exit waiter failed: {error}")))?
}
...
mod tests {
    ...
    /// An explicitly empty `HOME`/`CLAUDE_CONFIG_DIR` is refused before spawning
    ...
    fn empty_claude_config_dir_is_rejected() {
        let rejection = rejection_for("CLAUDE_CONFIG_DIR", std::ffi::OsString::new());
    ...
    fn assembled_credential_home_is_never_relative() {
        let assembled = allowlisted_environment(|queried| {
            (queried == "CLAUDE_CONFIG_DIR").then(|| std::ffi::OsString::from("relative-home"))
        })
