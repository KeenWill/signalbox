...
fn canonical_dollar_amount_matches_the_wire_decimal_representation() {
    assert!(CanonicalDollarAmount::try_new(String::from("79228162514264337593543950335")).is_ok());
    assert!(CanonicalDollarAmount::try_new(String::from("0.0000000000000000000000000001")).is_ok());
    assert!(CanonicalDollarAmount::try_new(String::from("79228162514264337593543950336")).is_err());
}
...
#[test]
fn uncorrelated_identity_is_reserved_for_server_errors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ServerFrame::try_new(RequestId::uncorrelated(), ServerMessage::SessionsStart {},).is_err()
    );
    assert!(
        ServerFrame::try_new(
            RequestId::uncorrelated(),
