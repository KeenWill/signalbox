...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let conservative =
        ServerFrame::try_new_for_version(ProtocolVersion::One, request(9)?, imported_conservative)?;
    let encoded_conservative = encode_server_line(&conservative)?;
    assert!(
        String::from_utf8(encoded_conservative.clone())?
            .contains(r#""source_speaker":{"type":"not_attested"},"content_kind":"tool_result""#)
    );
