...
fn wrong_typed_request_fields_fail_explicitly() {
    assert_client_malformed(r#"{"version":1,"request_id":1,"request":{"type":"list_sessions"}}"#);
}
...
fn container_depth_beyond_the_limit_is_malformed() {
    ...
    let error = decode_client_line(&line(&future)).expect_err("excessive nesting must be rejected");
