...
///
/// Version seven is reserved by the open owner turn-reconciliation stack
/// (`agent/turn-reconciliation-recovery`) and is admitted only when that stack
/// lands its `reconcile_turn` vocabulary.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.kind {
            ...
            FrameDecodeErrorKind::UnsupportedVersion => formatter.write_str(
                "process-protocol version is unsupported; supported versions are 1, 2, 3, 4, 5, 6, and 8",
            ),
        }
    }
}
...
fn probe_header(
    ...
    if !matches!(version_spelling, "1" | "2" | "3" | "4" | "5" | "6" | "8") {
        return Err(FrameDecodeError {
...
mod tests {
    ...
    fn assert_unsupported_version(version: &str) {
        ...
        assert!(
            error
                .to_string()
                .contains("supported versions are 1, 2, 3, 4, 5, 6, and 8")
        );
    }
    ...
    fn inv033_unsupported_version_precedes_payload_decoding() {
        ...
        // Version seven remains reserved by the open owner turn-reconciliation
        // stack and is not admitted until that stack lands.
