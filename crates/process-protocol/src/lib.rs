    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.kind {
            ...
            FrameDecodeErrorKind::UnsupportedVersion => formatter.write_str(
                "process-protocol version is unsupported; supported versions are 1 through 12, 14, and 16",
            ),
        }
    }
}
...
mod tests {
    ...
    fn assert_unsupported_version(version: &str) {
        ...
        assert!(
            error
                .to_string()
                .contains("supported versions are 1 through 12, 14, and 16")
        );
    }
    ...
    /// twelve plus fourteen and sixteen; thirteen and fifteen remain reserved.
    #[test]
    fn inv033_versions_fourteen_and_sixteen_complete_the_admitted_set() {
        assert_eq!(
        ...
        assert_eq!(
            ProtocolVersion::from_u64(14),
            Some(ProtocolVersion::Fourteen)
        );
