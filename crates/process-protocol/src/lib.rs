    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.kind {
            ...
            FrameDecodeErrorKind::UnsupportedVersion => formatter.write_str(
                "process-protocol version is unsupported; supported versions are 1 through 13",
            ),
        }
    }
}
...
mod tests {
    ...
    fn assert_unsupported_version(version: &str) {
        ...
        assert!(
            error
                .to_string()
                .contains("supported versions are 1 through 13")
        );
    }
    ...
    /// thirteen, with streaming at twelve and input delivery at thirteen.
    #[test]
    fn inv033_versions_twelve_and_thirteen_complete_the_admitted_set() {
        assert_eq!(
