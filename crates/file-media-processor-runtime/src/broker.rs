...
impl RangeBroker {
    pub(crate) const fn new(source_bytes: u64, envelope: WireReadEnvelope) -> Self {
        Self {
            source_bytes,
            envelope,
            cumulative_bytes: 0,
    ...
    Value: Serialize,
{
    ...
    if encoded.len() > MAX_PROCESSOR_FRAME_BYTES {
    ...
    Value: DeserializeOwned,
{
    ...
    if length == 0 || length > MAX_PROCESSOR_FRAME_BYTES {
        return Err(BrokerError::Frame);
    }
