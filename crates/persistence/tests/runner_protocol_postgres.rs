...
const COUNTERFEIT_PROMOTION_COMMAND: u128 = 0x9367;
...
async fn stage_relational_promotion(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    fixture: &PendingPromotionFixture,
    command: u128,
    registration_revision: RunnerGeneration,
) -> Result<(), sqlx::Error> {
    ...
    .bind(fixture.candidate_request.into_uuid())
    .fetch_one(&mut **transaction)
    .await?;
    let connection_epoch: Decimal = evidence.try_get("connection_epoch")?;
    let connection_event_ordinal: Decimal = evidence.try_get("connection_event_ordinal")?;
    let predecessor_enrollment_id: Uuid = evidence.try_get("predecessor_enrollment_id")?;
    let predecessor_loss_epoch: Decimal = evidence.try_get("predecessor_loss_epoch")?;
    sqlx::query(
        "INSERT INTO runner_enrollment_audit
            (enrollment_id, revision, runner_id,
             authentication_reference_id, allowed_class_count, state_kind)
    ...
    .bind(fixture.predecessor_enrollment.into_uuid())
    .execute(&mut **transaction)
    .await?;
    sqlx::query(
        "INSERT INTO runner_enrollment_audit_allowed_class
    ...
    sqlx::query(
        "INSERT INTO promote_pending_runner_command
            (command_id, pending_request_id, result_kind,
             ...
             result_registration_revision, result_connection_epoch,
             result_connection_event_ordinal, predecessor_enrollment_id,
             predecessor_loss_epoch)
         VALUES ($1, $2, 'applied', $3, $4, $5, $6, $7, $8, $9)",
    )
    .bind(uuid(command))
    .bind(fixture.candidate_request.into_uuid())
    .bind(fixture.candidate_enrollment.into_uuid())
    .bind(fixture.candidate_runner.into_uuid())
    .bind(Decimal::from(registration_revision.get()))
    .bind(connection_epoch)
    .bind(connection_event_ordinal)
    .bind(predecessor_enrollment_id)
...
async fn workspace_provisioning_authorization_fixture(
    pool: &PgPool,
    credential_profile: Option<CredentialProfileName>,
    ...
    let working_directory = RunnerWorkingDirectory::try_new(
        "/workspace/replacement-authorization-predecessor".to_owned(),
    )
    .expect("the fixture workspace directory is valid");
    let workspace = ProvisionedWorkspace {
        ...
        .expect("the fixture repository path is relative"),
    ...
    let pin = placement
        ...
        .expect("the repository placement pins with its complete workspace");
    ...
    let successor_registration = store.register(&successor, fixture_advertisement).await?;
    ...
    .bind(successor.enrollment().into_uuid())
    .fetch_one(pool)
    .await?;
    Ok(WorkspaceProvisioningAuthorizationFixture {
        ...
        lost_placement_event_ordinal: lost_placement_event_ordinal
            .to_u64()
            .expect("the fixture event ordinal is an unsigned integer"),
        lost_placement_revision: RunnerGeneration::one(),
        successor_placement_revision: RunnerGeneration::try_from_u64(2)
            .expect("the successor revision is positive"),
        ...
        connection_event_ordinal: connection_event_ordinal
            .to_u64()
            .expect("the fixture connection event ordinal is unsigned"),
        repository,
        sandbox,
        credential_profile,
    })
...
async fn insert_workspace_provisioning_authorization_projection(
    pool: &PgPool,
    fixture: &WorkspaceProvisioningAuthorizationFixture,
    repository: &WorkspaceRepositoryKey,
) -> Result<(), sqlx::Error> {
    ...
    .bind(Decimal::from(fixture.lost_placement_revision.get()))
    .bind(fixture.runner.into_uuid())
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
    ...
    .bind(fixture.command.into_uuid())
    .bind(fixture.session.into_uuid())
    .bind(Decimal::from(fixture.lost_placement_event_ordinal))
    .bind(Decimal::from(fixture.lost_placement_revision.get()))
    .bind(Decimal::from(fixture.successor_placement_revision.get()))
    .bind(fixture.enrollment.into_uuid())
    .bind(fixture.runner.into_uuid())
    .bind(Decimal::from(fixture.registration_revision.get()))
...
async fn install_offer_registration_compatibility_column(pool: &PgPool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "ALTER TABLE runner_lease_generation
         ADD COLUMN offer_registration_revision numeric(20, 0)",
    )
...
async fn drop_offer_registration_compatibility_column(pool: &PgPool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "ALTER TABLE runner_lease_generation
...
async fn insert_terminal_connection_loss_before_cursor_migration(
    pool: &PgPool,
    enrollment: RunnerEnrollmentId,
    connection: RunnerConnectionEpoch,
) -> Result<(), sqlx::Error> {
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "INSERT INTO runner_connection_event
            (enrollment_id, connection_epoch, event_ordinal,
             state_kind, cause_kind)
         VALUES ($1, $2, 2, 'lost', 'transport_closed')",
    )
    .bind(enrollment.into_uuid())
    .bind(Decimal::from(connection.get()))
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
    ...
    sqlx::query(
        "UPDATE runner_connection_authority_head
            SET connection_event_ordinal = 2, latest_loss_epoch = 1
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv042_inv044_pending_successor_promotion_installs_atomic_state()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv001_inv042_promoted_enrollment_replays_active_authority() -> Result<(), Box<dyn Error>>
{
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    fixture
...
/// INV-001 / INV-042: replay reconstitutes and validates the immutable
/// connected-event evidence retained by an applied promotion.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv001_inv042_promoted_enrollment_replay_rejects_corrupt_connection_evidence()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    ...
    repository.handle(command).await?;
    fixture
        .store
        .transition_connection(
            fixture.candidate_enrollment,
        ...
        .await?;
    sqlx::query(
        "ALTER TABLE promote_pending_runner_command
         DISABLE TRIGGER promote_pending_runner_command_is_append_only",
    )
    .execute(&pool)
    .await?;
    sqlx::query(
        "UPDATE promote_pending_runner_command AS command
            SET result_connection_epoch = authority.connection_epoch,
                result_connection_event_ordinal =
                    authority.connection_event_ordinal
           FROM runner_connection_authority_head AS authority
          WHERE command.command_id = $1
            AND authority.enrollment_id = command.result_enrollment_id",
    )
    .bind(command.command().into_uuid())
    .execute(&pool)
    .await?;
    sqlx::query(
        "ALTER TABLE promote_pending_runner_command
         ENABLE TRIGGER promote_pending_runner_command_is_append_only",
    )
    .execute(&pool)
    .await?;
    ...
    let corrupted = repository
        .handle(command)
        .await
        .expect_err("replay rejects retained evidence that names a lost event");

    assert_eq!(
        corrupted.to_string(),
        "runner promotion storage is corrupt: applied promotion evidence"
    );
    drop(pool);
    Ok(())
}

/// INV-042: a promoted enrollment retains the ordinary terminal revocation
/// transition at revision three.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv042_promoted_enrollment_revokes_at_revision_three() -> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    fixture
        .store
    ...
    assert_eq!(revision, Decimal::from(3));
    drop(pool);
    Ok(())
}
    ...
    sqlx::query(
        "INSERT INTO promote_pending_runner_command
            (command_id, pending_request_id, result_kind,
             ...
             result_connection_event_ordinal, predecessor_enrollment_id,
             predecessor_loss_epoch)
         VALUES ($1, $2, 'applied', $3, $4, $5, 1, 1, $6, 1)",
    )
    .bind(command.into_uuid())
...
/// INV-042: an applied promotion requires the candidate's exact revision-one
/// pending and revision-two active audit transition.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv042_applied_promotion_rejects_counterfeit_candidate_audit() -> Result<(), Box<dyn Error>>
{
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    fixture
    ...
    sqlx::query(
        "ALTER TABLE runner_enrollment_audit
         DISABLE TRIGGER runner_enrollment_audit_is_append_only",
    )
    .execute(&pool)
    .await?;
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "UPDATE runner_enrollment_audit
    ...
    .execute(&mut *transaction)
    .await?;
    stage_relational_promotion(
        &mut transaction,
        &fixture,
        COUNTERFEIT_PROMOTION_COMMAND,
        fixture.candidate_registration,
    )
    .await?;
    ...
    let rejected = transaction
        .commit()
        .await
        .expect_err("an active-at-creation candidate cannot counterfeit promotion");
    sqlx::query(
        "ALTER TABLE runner_enrollment_audit
         ENABLE TRIGGER runner_enrollment_audit_is_append_only",
    )
    .execute(&pool)
    .await?;
...
/// INV-042: relational promotion rows cannot apply while the pending candidate
/// has no current connected authority.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv042_applied_promotion_rejects_disconnected_candidate() -> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    ...
    stage_relational_promotion(
        &mut transaction,
        &fixture,
        DISCONNECTED_PROMOTION_COMMAND,
        fixture.candidate_registration,
    )
    .await?;
    ...
    let rejected = transaction
        .commit()
        .await
        .expect_err("a disconnected candidate cannot be recorded as promoted");
...
/// INV-042: relational promotion rows cannot apply after the predecessor's
/// loss authority has been superseded by a connected epoch.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv042_applied_promotion_rejects_reconnected_predecessor() -> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    fixture
        .store
    ...
    let rejected = transaction
        .commit()
        .await
        .expect_err("a reconnected predecessor invalidates promotion authority");
...
/// INV-001 / INV-042: an applied result retains the pending request's exact
/// registration rather than a later registration on the same enrollment.
#[tokio::test]
#[ignore = "requires Docker"]
async fn inv001_inv042_applied_promotion_rejects_later_registration() -> Result<(), Box<dyn Error>>
{
    let (_container, pool) = migrated_postgres().await?;
    let fixture = pending_promotion_fixture(pool.clone()).await?;
    fixture
    ...
    let replay = fixture
        ...
        .await?;
    let later_registration = fixture
        .store
        .resume_registration(
            replay.receipt().request(),
            replay.receipt().identities(),
            replay.receipt().registration().revision(),
            narrowed_advertisement(),
        )
        .await?;
    let mut transaction = pool.begin().await?;
    sqlx::query("DROP INDEX promote_pending_runner_one_applied_result")
        .execute(&mut *transaction)
        .await?;
    sqlx::query(
        "INSERT INTO durable_command
    ...
    .bind(uuid(CROSS_WIRED_PROMOTION_COMMAND))
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
        "INSERT INTO promote_pending_runner_command
            (command_id, pending_request_id, result_kind,
             ...
             result_connection_event_ordinal, predecessor_enrollment_id,
             predecessor_loss_epoch)
         SELECT $1, pending_request_id, 'applied', result_enrollment_id,
                result_runner_id, $2, result_connection_epoch,
                result_connection_event_ordinal, predecessor_enrollment_id,
                predecessor_loss_epoch
           FROM promote_pending_runner_command
          WHERE command_id = $3",
    ...
    .bind(uuid(CROSS_WIRED_PROMOTION_COMMAND))
    .bind(Decimal::from(
        later_registration.registration().revision().get(),
    ))
    .bind(uuid(PROMOTE_PENDING_RUNNER_COMMAND))
    .execute(&mut *transaction)
    .await?;
    ...
    let rejected = transaction
        .commit()
        .await
        .expect_err("promotion cannot retain a later registration as its result");
...
async fn runner_loss_migration_rejects_legacy_rows_without_source() -> Result<(), Box<dyn Error>> {
    ...
    sqlx::query(
        "ALTER TABLE runner_session_placement_record
            ...
            ADD CONSTRAINT runner_session_placement_event_closed CHECK (TRUE),
            ADD CONSTRAINT runner_session_placement_state_shape CHECK (TRUE),
            ADD CONSTRAINT runner_session_placement_workspace_shape CHECK (TRUE)",
    )
    .execute(&pool)
    .await?;
    sqlx::query(
        "INSERT INTO runner_session_placement_record
            (session_id, event_ordinal, placement_revision, event_kind,
             ...
             workspace_requirement_kind, requested_sandbox_profile,
             permission_override_count, state_kind, pinned_tool_count)
         VALUES ($1, 1, 1, 'runner_lost', 'identity', $2,
                 'runner_default', 'none', 'ambient', 0, 'runner_lost', 0)",
    )
    .bind(uuid(SESSION))
    .bind(uuid(RUNNER))
    .execute(&pool)
    ...
    let refusal = migrate(&pool)
        .await
        .expect_err("legacy runner loss must fail before source facts are invented");
    let applied_version: Option<i64> =
        sqlx::query_scalar("SELECT max(version) FROM _sqlx_migrations")
            .fetch_one(&pool)
            .await?;
    ...
    assert!(
        refusal.to_string().contains(LEGACY_PLACEMENT_LOSS_REFUSAL),
...
-> Result<(), Box<dyn Error>> {
    ...
    MIGRATOR
        .run_to(PRE_PLACEMENT_LOSS_FENCE_MIGRATION, &pool)
        .await?;
    install_offer_registration_compatibility_column(&pool).await?;
    ...
    .await?;
    drop_offer_registration_compatibility_column(&pool).await?;
    let refusal = migrate(&pool)
        .await
...
/// INV-044: losses predating bounded propagation migrate to an exact completed
/// cursor because their placements received the compatibility loss baseline.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv044_runner_loss_cursor_migration_backfills_completed_state()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = unmigrated_postgres().await?;
    MIGRATOR
    ...
    let loss = store
        .load_current_connection_loss(expected_enrollment.enrollment())
        .await?
        .expect("the migrated terminal connection retains its loss");
...
-> Result<(), Box<dyn Error>> {
    ...
    let loaded = RunnerProtocolStore::new(pool.clone(), catalog())
        ...
        .await?
        .expect("the valid pre-migration placement remains current");

    assert_eq!(loaded.placement(), &expected);
...
-> Result<(), Box<dyn Error>> {
    ...
    let pin = placement
        .pin_and_offer_lease(
            ...
            registration.registration(),
            RunnerWorkingDirectory::try_new("/workspace/session".to_owned())
                .expect("the pinned directory is valid"),
            None,
            authorized(INITIAL_PHYSICAL_ATTEMPT),
            offer_request(),
        )
        .expect("the pre-migration fixture pins without a credential grant");
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "INSERT INTO runner_session_placement_record
            (session_id, event_ordinal, placement_revision, event_kind,
             ...
             requested_sandbox_profile, permission_override_count,
             state_kind, pinned_tool_count)
         VALUES ($1, 1, 1, 'created', 'identity', $2, 'exact', $3,
                 'none', 'workspace_restricted', 0, 'unpinned', 0)",
    )
    .bind(pin.placement.session().into_uuid())
    .bind(expected_enrollment.runner().into_uuid())
    .bind("/workspace/session")
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
        "INSERT INTO runner_current_session_placement (session_id, event_ordinal)
    ...
    sqlx::query(
        "ALTER TABLE runner_session_placement_record
         DISABLE TRIGGER runner_initial_pin_requires_lease",
    )
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
        "INSERT INTO runner_session_placement_record
            (session_id, event_ordinal, placement_revision, event_kind,
             ...
             registration_enrollment_id, registration_revision,
             pinned_tool_count)
         SELECT $1, 2, 1, 'pinned', 'identity', $2, 'exact', $3,
                'none', 'workspace_restricted', 0, 'pinned', $2, $3,
                $4, $5, count(*)
           FROM runner_registration_tool
          WHERE enrollment_id = $4 AND registration_revision = $5",
    )
    .bind(pin.placement.session().into_uuid())
    .bind(expected_enrollment.runner().into_uuid())
    .bind("/workspace/session")
    ...
    sqlx::query(
        "INSERT INTO runner_session_placement_tool
         ...
         SELECT $1, 2, tool_name, loci_kind = 'runner_only'
           FROM runner_registration_tool
          WHERE enrollment_id = $2 AND registration_revision = $3",
    )
    .bind(pin.placement.session().into_uuid())
    .bind(expected_enrollment.enrollment().into_uuid())
    .bind(Decimal::from(registration.revision().get()))
    ...
    let loaded = RunnerProtocolStore::new(pool.clone(), catalog())
        .load_placement(pin.placement.session())
        .await?
        .expect("the valid pinned pre-migration placement remains current");
...
-> Result<(), Box<dyn Error>> {
    ...
    .fetch_one(&pool)
    .await?;
    let entry = SemanticTranscriptEntryId::from_uuid(uuid(0xb010));
    let frontier = ContextFrontierId::from_uuid(uuid(0xb011));
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "INSERT INTO semantic_transcript_entry
            (source_session_id, semantic_entry_id, payload_kind,
    ...
    .execute(&mut *transaction)
    .await?;
    let non_prefix = transaction
        .commit()
        .await
        .expect_err("an existing semantic prefix cannot be replaced by a new root");

    assert_check_violation(non_prefix);
    drop(pool);
    Ok(())
}

/// S32 / INV-015 / INV-044: a placement boundary extends the current physical
/// frontier leaf and cannot branch from an already-extended prefix.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv015_inv044_runner_placement_boundary_rejects_stale_frontier_tip()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let (store, _, registration, pin) = stored_pin_fixture(&pool).await?;
    let original_grant = pin
    ...
    .fetch_one(&pool)
    .await?;
    let stale_prefix = ContextFrontierId::from_uuid(uuid(0xb020));
    let authoritative_tip = ContextFrontierId::from_uuid(uuid(0xb021));
    let entry = SemanticTranscriptEntryId::from_uuid(uuid(0xb022));
    let stale_successor = ContextFrontierId::from_uuid(uuid(0xb023));
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "INSERT INTO context_frontier
            (owning_session_id, context_frontier_id, member_count)
    ...
    .bind(session.into_uuid())
    .bind(authoritative_tip.into_uuid())
    .bind(stale_prefix.into_uuid())
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
    ...
    .bind(session.into_uuid())
    .bind(stale_successor.into_uuid())
    .bind(stale_prefix.into_uuid())
    .execute(&mut *transaction)
    .await?;
    sqlx::query(
    ...
    .execute(&mut *transaction)
    .await?;
    let stale_boundary = transaction
        .commit()
        .await
        .expect_err("a placement boundary cannot branch from a stale frontier tip");

    assert_check_violation(stale_boundary);
    drop(pool);
    Ok(())
}
...
/// S32 / INV-044: staged replacement provisioning retains and independently
/// reads every command, loss-boundary, runner, and workspace correlation.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv044_profileless_workspace_provisioning_authorization_round_trips()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = workspace_provisioning_authorization_fixture(&pool, None).await?;
    insert_workspace_provisioning_authorization_projection(&pool, &fixture, &fixture.repository)
        .await?;
    let loaded = fixture
        .store
        .load_workspace_provisioning_authorization(fixture.authorization)
        .await?
        .expect("the checked authorization reads back");

    assert_eq!(loaded.command(), fixture.command);
    assert_eq!(loaded.authorization(), fixture.authorization);
    assert_eq!(loaded.session(), fixture.session);
    assert_eq!(
        loaded.lost_placement_event_ordinal(),
        fixture.lost_placement_event_ordinal
    );
    assert_eq!(
        loaded.lost_placement_revision(),
        fixture.lost_placement_revision
    );
    assert_eq!(
        loaded.successor_placement_revision(),
        fixture.successor_placement_revision
    );
...
/// S32 / INV-044: the optional credential arm reads back the exact profile
/// selected by both the placement and repository entry.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv044_profiled_workspace_provisioning_authorization_round_trips()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    ...
    let loaded = fixture
        ...
        .load_workspace_provisioning_authorization(fixture.authorization)
        .await?
        .expect("the profiled authorization reads back");

    assert_eq!(loaded.authorization(), fixture.authorization);
    assert_eq!(loaded.credential_profile(), Some(&selected_profile));
    assert_eq!(loaded.repository(), &fixture.repository);
    drop(pool);
    Ok(())
}

/// S32 / INV-044: readback rejects a stored sandbox that no longer matches the
/// exact lost placement even when the selected registration advertises it.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv044_workspace_provisioning_authorization_readback_rejects_cross_wired_sandbox()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = workspace_provisioning_authorization_fixture(&pool, None).await?;
    insert_workspace_provisioning_authorization_projection(&pool, &fixture, &fixture.repository)
        .await?;
    sqlx::query(
        "ALTER TABLE runner_workspace_provisioning_authorization
         DISABLE TRIGGER runner_workspace_provisioning_authorization_is_append_only",
    )
    .execute(&pool)
    .await?;
    sqlx::query(
        "UPDATE runner_workspace_provisioning_authorization
            SET sandbox_profile = 'workspace_restricted'
    ...
    sqlx::query(
        "ALTER TABLE runner_workspace_provisioning_authorization
         ENABLE TRIGGER runner_workspace_provisioning_authorization_is_append_only",
    )
    .execute(&pool)
    .await?;
    let corrupted = fixture
        ...
        .load_workspace_provisioning_authorization(fixture.authorization)
        .await
        .expect_err("the read boundary rejects a cross-wired sandbox");
...
/// S32 / INV-044: the relational boundary rejects repository authority absent
/// from both the lost placement and the selected registration.
#[tokio::test]
#[ignore = "requires Docker"]
async fn s32_inv044_workspace_provisioning_authorization_rejects_a_cross_wired_repository()
-> Result<(), Box<dyn Error>> {
    let (_container, pool) = migrated_postgres().await?;
    let fixture = workspace_provisioning_authorization_fixture(&pool, None).await?;
    let foreign_repository = WorkspaceRepositoryKey::try_new("foreign".to_owned())
        .expect("the foreign fixture repository key is valid");
    let rejected = insert_workspace_provisioning_authorization_projection(
        &pool,
        &fixture,
        &foreign_repository,
    )
    .await
    .expect_err("the authorization cannot select a foreign repository");
