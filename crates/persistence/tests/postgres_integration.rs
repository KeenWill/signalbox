...
async fn assert_failed_judge_escalates_to_user(
    pool: &PgPool,
    seed: u128,
    disposition: &str,
) -> Result<(), Box<dyn Error>> {
    ...
    let command = DurableCommandId::from_uuid(Uuid::from_u128(seed + 0x20));
    PostgresToolLoopRepository::new(pool.clone())
        .decide(
            decide_tool_request(command, *request, ToolApprovalDecision::Approve),
            || TurnAttemptId::from_uuid(Uuid::from_u128(seed + 0x21)),
        )
        .await?;
    let (event_turn, approval) = dispatched_tool_approval_decision(pool, *request)
        .await?
        .expect("a failed judge call escalates the parked request to the user");
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires ephemeral PostgreSQL"]
async fn approval_judge_call_appears_in_model_call_history_with_provenance()
-> Result<(), Box<dyn Error>> {
    let (container, pool, _database_url) = migrated_postgres().await?;
    let seed = 0x7ea0;
    let (fixture, _, _, requests) = checkpoint_tool_batch_with_approval(
    ...
    let mut connection = pool.acquire().await?;
    let (_selection, judge_call) = insert_completed_judge(
        &mut connection,
        &fixture,
        *request,
        seed + 0x10,
        "escalate_to_human",
        Some(Decimal::from(11_u64)),
    )
    .await?;
    drop(connection);
    ...
    let snapshot = ProcessReadRepository::new(pool.clone())
        .read_transcript(fixture.session)
        .await?
        .expect("the parked turn has a transcript projection");
    let judge_usage = snapshot
        .model_call_usage()
        .iter()
        .find(|usage| usage.call() == ModelCallId::from_uuid(judge_call))
        .expect("the terminal approval judge appears in model-call history");
    assert_eq!(
        judge_usage.target(),
    ...
    assert_eq!(judge_usage.credential_profile(), "fixture-credential");
    assert_eq!(
        judge_usage.provenance(),
        ProcessModelCallUsageProvenance::Reported
    );
    assert_eq!(
        judge_usage.input_token_semantics(),
        Some(ProcessModelCallInputTokenSemantics::CacheExclusive)
    );
    assert_eq!(judge_usage.usage().input_tokens(), Some(11));
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires ephemeral PostgreSQL"]
async fn approval_guard_each_failed_judge_disposition_escalates_to_user()
-> Result<(), Box<dyn Error>> {
    let (container, pool, _database_url) = migrated_postgres().await?;
    assert_failed_judge_escalates_to_user(&pool, 0x7eb0, "known_failed").await?;
    assert_failed_judge_escalates_to_user(&pool, 0x7ec0, "refused").await?;
    assert_failed_judge_escalates_to_user(&pool, 0x7ed0, "cancelled").await?;
    assert_failed_judge_escalates_to_user(&pool, 0x7ee0, "ambiguous").await?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let error = transaction
        .commit()
        .await
        .expect_err("delegate decisions preserve proposal order");
    assert_eq!(
        database_constraint(&error),
        Some("tool_approval_delegate_requires_checked_judge")
    );
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires ephemeral PostgreSQL"]
async fn approval_guard_automatic_approval_cannot_emit_an_explicit_decision_event()
-> Result<(), Box<dyn Error>> {
    let (container, pool, _database_url) = migrated_postgres().await?;
    let (fixture, _, _, requests) = checkpoint_tool_batch_with_approval(
        &pool,
    ...
    .await?;
    let [request] = requests.as_slice() else {
        panic!("the fixture has one automatic request")
    };
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "WITH header AS (
    ...
    let error = transaction
        .commit()
        .await
        .expect_err("automatic approval has no explicit decider");
    assert_eq!(
        database_constraint(&error),
