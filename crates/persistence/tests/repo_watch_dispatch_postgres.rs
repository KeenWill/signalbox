...
use signalbox_domain::{
    AcceptedInputId, BranchName, CheckConclusion, CheckRunName, CommitSha,
    DangerousToolAutoApproval, DescendantTerminationScope, DirectModelSelection, DurableCommandId,
    GitHubObjectId, GoalCommandResult, GoalModelProvenance, GoalNeed, GoalReport,
    ...
    let committed = event_store
        .commit(
            ...
            RepoWatchCommitRequest::new(
                Some(cursor.generation()),
                RepoWatchCursorCandidate::new(observation),
                vec![mergeable_event(event_id, head, MergeableState::Mergeable)?],
            ),
        )
        .await?;
