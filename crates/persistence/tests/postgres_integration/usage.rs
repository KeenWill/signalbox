...
-> Result<(), Box<dyn Error>> {
    ...
    assert!(titles.unclaimed_initial_turns(None, 64).await?.is_empty());
    ...
    assert_eq!(
        titles.unclaimed_initial_turns(None, 64).await?,
        vec![(fixture.session, fixture.turn)]
    );
    assert!(
        titles
            .unclaimed_initial_turns(Some(fixture.session), 1)
            .await?
            .is_empty()
    );
    assert!(titles.unclaimed_initial_turns(None, 0).await?.is_empty());
