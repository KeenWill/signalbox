...
pub(crate) async fn inspect(
    ...
    row.map(|row| {
        ...
        match (kind, has_create, has_defaults) {
            (CommandKind::CreateSession, true, false)
            | (CommandKind::ReplaceSessionDefaults, false, true) => Ok(kind),
            (CommandKind::CreateSession, false, false)
            | (CommandKind::ReplaceSessionDefaults, false, false) => Err(
