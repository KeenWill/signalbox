...
pub(super) async fn load_tool_conversation_entries(
    ...
    let context_texts = sqlx::query_as::<_, (Uuid, Option<String>, Option<String>)>(
        "SELECT attempt_id, context_result_text, context_error_detail FROM tool_attempt WHERE attempt_id = ANY($1)",
    )
    .bind(
        attempt_ids
            .iter()
    ...
    .await?
    .into_iter()
    .map(|(id, text, detail)| (id, (text, detail)))
        ...
        .filter(|entry| projected.contains(&entry.reference()))
    {
        ...
        match entry.payload() {
            ...
            SemanticTranscriptEntryPayload::ToolExecutionResult { attempt } => {
                ...
                let (context_text, context_error_detail) = context_texts
                ...
                resolved.push(ResolvedToolConversationEntry::ExecutionResult {
                    ...
                    context_error_detail: context_error_detail
                        .map(signalbox_domain::ToolExecutionErrorDetail::try_new)
                        .transpose()
                        .map_err(|_| ModelCallCorruption::Inconsistent("tool context error"))?,
                });
            }
            SemanticTranscriptEntryPayload::ToolDenied {
                request: request_id,
