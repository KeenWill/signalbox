-- Generalize bounded ambiguity reconciliation to model calls and tool attempts.

ALTER TABLE automatic_model_call_reconciliation
    RENAME TO automatic_reconciliation;
ALTER TABLE automatic_model_call_reconciliation_attempt
    RENAME TO automatic_reconciliation_attempt;
ALTER TABLE automatic_model_call_reconciliation_discovery_state
    RENAME TO automatic_reconciliation_discovery_state;
ALTER TABLE automatic_model_call_reconciliation_supersession_state
    RENAME TO automatic_reconciliation_supersession_state;
ALTER INDEX automatic_model_call_reconciliation_due
    RENAME TO automatic_reconciliation_due;

ALTER TABLE automatic_reconciliation
    ALTER COLUMN model_call_id DROP NOT NULL,
    ADD COLUMN tool_attempt_id uuid,
    ADD CONSTRAINT automatic_reconciliation_tool_attempt
        FOREIGN KEY (tool_attempt_id, turn_id, session_id)
        REFERENCES tool_attempt (attempt_id, turn_id, session_id),
    ADD CONSTRAINT automatic_reconciliation_operation
        CHECK (num_nonnulls(model_call_id, tool_attempt_id) = 1);

-- The bounded discovery and supersession scans now cover both ambiguous
-- shapes, so their covering indexes carry the tool-attempt identity and admit
-- the tool-recovery phase. Recreated rather than renamed: a rename keeps the
-- old predicate and INCLUDE list, which is exactly what changed.
DROP INDEX automatic_model_call_reconciliation_supersession;
CREATE INDEX automatic_reconciliation_supersession
    ON automatic_reconciliation (turn_id)
    INCLUDE (session_id, model_call_id, tool_attempt_id, state_kind,
             attempt_count)
    WHERE state_kind IN ('scheduled', 'attempting', 'exhausted');

DROP INDEX turn_lifecycle_automatic_model_call_recovery_discovery;
CREATE INDEX turn_lifecycle_automatic_recovery_discovery
...
BEGIN
    IF workflow_schema IS NULL THEN
        RAISE EXCEPTION
            'automatic operation reconciliation migration requires a current schema';
    END IF;
...
-- The delegated payload and result validators read the durable recovery table
-- by name, which plpgsql resolves when they run rather than when they are
-- created. Renaming the table above therefore leaves them naming a relation
-- that no longer exists, so both are recreated here against the new name; the
