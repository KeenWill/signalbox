CREATE OR REPLACE FUNCTION context_frontier_member_position(checked_session_id uuid, checked_frontier_id uuid, checked_source_session_id uuid, checked_semantic_entry_id uuid) RETURNS numeric
    LANGUAGE sql STABLE
    AS $$
...
CREATE OR REPLACE FUNCTION require_context_compaction_exact_evidence() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    ELSE
        ...
        ) CYCLE context_compaction_id SET cycle USING visited
        SELECT
            chain.through_source_session_id,
            chain.through_entry_id
          INTO consumed_through_source_session, consumed_through_entry
          FROM predecessor_chain AS chain
         WHERE NOT EXISTS (SELECT 1 FROM predecessor_chain WHERE cycle)
           AND NOT EXISTS (
                SELECT 1
                  FROM context_compaction AS consumed_summary
                 WHERE consumed_summary.session_id = chain.through_source_session_id
                   AND consumed_summary.summary_entry_id = chain.through_entry_id
         )
        ...
        IF ROW(NEW.through_source_session_id, NEW.through_entry_id) <>
           ...
           AND (
                ...
                OR EXISTS (
                    SELECT 1
                      FROM context_compaction AS consumed_summary
                     WHERE consumed_summary.session_id = NEW.through_source_session_id
            ...
            SELECT member.source_session_id, member.semantic_entry_id
             ...
             WHERE ROW(NEW.through_source_session_id, NEW.through_entry_id) <>
               ...
               AND NOT EXISTS (
                    SELECT 1
                      FROM context_compaction AS consumed_summary
                     WHERE consumed_summary.session_id = member.source_session_id
