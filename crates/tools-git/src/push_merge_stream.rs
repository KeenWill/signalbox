...
pub(super) fn protected_base_hunks(
    ...
    protected.edits.clone_from(&main.edits);
    for &b in &main.edits {
        check(deadline)?;
        ...
        for &a in &own.edits {
            ...
            if !generated && a[..2] == b[..2] {
                let mut own_text = tempfile::tempfile().map_err(failed)?;
                let mut base_text = tempfile::tempfile().map_err(failed)?;
                append_lines(branch, a[2], a[3], &mut own_text, deadline)?;
                append_lines(base, b[2], b[3], &mut base_text, deadline)?;
                if equal_files(
                    &own_text,
                    0,
                    own_text.metadata().map_err(failed)?.len(),
                    &base_text,
                    0,
                    base_text.metadata().map_err(failed)?.len(),
                    deadline,
                )? {
                    continue;
                }
...
pub(super) fn preserves_indexed_text(
    ...
    let [ancestor, branch_lines, base_lines] = lines;
    let generated = path.extension().is_some_and(|extension| extension == "md")
        && generated_marker(base_lines)?;
    ...
    while index < edits.len() {
        ...
        index += 1;
        while index < edits.len()
            && overlapping_edits(&(start..end), &(edits[index].1[0]..edits[index].1[1]))
        {
...
mod tests {
    ...
    fn fixture_regeneration_accepts_json_lines_larger_than_the_diff_preview() {
        ...
        let mut own = Hunks::new().expect("branch hunks");
        let mut main = Hunks::new().expect("base hunks");
        let mut merged = Hunks::new().expect("result hunks");
        assert!(matches!(
            text_hunks(content(&ancestor), content(&branch), &mut own, deadline)
                .expect("branch diff"),
            TextDiff::Detailed
        ));
        assert!(matches!(
            text_hunks(content(&ancestor), content(&base), &mut main, deadline).expect("base diff"),
            TextDiff::Detailed
        ));
        assert!(matches!(
            text_hunks(content(&ancestor), content(&result), &mut merged, deadline)
                .expect("result diff"),
            TextDiff::Detailed
        ));
        let ancestor_lines = Lines::new(content(&ancestor), deadline)
            .expect("ancestor lines")
            .expect("ancestor index");
        let branch_lines = Lines::new(content(&branch), deadline)
            .expect("branch lines")
            .expect("branch index");
        let base_lines = Lines::new(content(&base), deadline)
            .expect("base lines")
            .expect("base index");
        let result_lines = Lines::new(content(&result), deadline)
            .expect("result lines")
            .expect("result index");
        let mut protected = protected_base_hunks(
            std::path::Path::new("tests/fixture.rs"),
            std::path::Path::new("tests/fixture.rs"),
            [&ancestor_lines, &branch_lines, &base_lines, &result_lines],
            [&own, &main, &merged],
            deadline,
        )
        ...
        let mut empty = Hunks::new()
            ...
            .permitted_filtered(Some(true), deadline)
            .expect("empty matching set");
        assert!(
            protected
                .first_dropped_filtered(&mut empty, MAX_MERGE_DETAIL_BYTES, Some(true), deadline)
    ...
    fn streamed_fixture_classifier_requires_a_complete_raw_json_literal() {
        ...
        for (name, literal, expected) in cases {
            let deadline = Instant::now() + Duration::from_secs(30);
            let lines = Lines::new(content(literal), deadline)
                .expect("literal lines")
