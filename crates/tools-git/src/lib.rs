...
use git2::{
    BranchType, Buf, Config, Delta, DiffFindOptions, DiffFormat, DiffOptions, ErrorCode, Index,
    IndexEntry, IndexTime, Indexer, Mempack, Odb, Patch, Repository, RepositoryOpenFlags,
    RepositoryState, Signature, build::CheckoutBuilder,
};
use rustix::fs::{
        ...
        for entry in entries {
            ...
            if file_type.is_dir() {
                ...
                if relative == Path::new("shallow") {
                    validate_shallow_file(&entry.path())?;
                }
            }
        }
    }
...
fn validate_shallow_file(path: &Path) -> Result<(), LocalGitToolsConstructionError> {
    let descriptor = openat(
        CWD,
        path,
        OFlags::RDONLY | OFlags::NONBLOCK | OFlags::NOFOLLOW | OFlags::CLOEXEC,
...
impl<FileSystem: WorkspaceFileSystem> LocalGitExecutor<FileSystem> {
    ...
    fn stage(
        ...
        let mempack = object_database
            .add_new_mempack_backend(1000)
            .map_err(|_| LocalGitFailure::Operation)?;
        self.stage_with_pinned_objects(repository, &object_database, &mempack, arguments)
    }

    fn stage_with_pinned_objects(
        &self,
        repository: &Repository,
    ...
    fn branch_switch(
        ...
        self.branch_switch_with_hook(repository, arguments, || {})
    }
...
fn restore_index(
    ...
    let (lock, _replacement) = IndexLock::acquire(index_path, index_lock_path)?;
    lock.write(index)?;
    lock.commit()
}
...
fn open_reference_parent(
    ...
    for component in parent_path.components() {
        ...
        directory = if create {
            open_or_create_ref_directory(&directory, component)?
        } else {
            openat(
                &directory,
                component,
...
impl IndexLock {
    fn acquire(index_path: &Path, lock_path: &Path) -> Result<(Self, Index), LocalGitFailure> {
        ...
        guard
            ...
            .map_err(|_| LocalGitFailure::Operation)?;
        let index =
            Index::open(&descriptor_path(&guard.lock)).map_err(|_| LocalGitFailure::Operation)?;
        Ok((guard, index))
    }
...
fn log(repository: &Repository, arguments: GitLogArguments) -> Result<LogResult, LocalGitFailure> {
    ...
    walk.set_sorting(git2::Sort::TOPOLOGICAL | git2::Sort::TIME)
        .and_then(|()| walk.push(start))
        .map_err(|_| LocalGitFailure::Operation)?;
    let ordered = walk
        .take(arguments.max_entries.saturating_add(1))
...
fn resolve_bounded_commit<'repository>(
    ...
    let mut oid = resolve_exact_revision_oid(repository, revision)?;
    for _depth in 0..16 {
...
fn resolve_exact_revision_oid(
    ...
    let (_, target) = resolve_reference_chain(repository, revision)?;
...
fn persist_objects(
    ...
    for root in roots {
        match root {
            PackRoot::Object(oid) => builder.insert_object(*oid, None),
            PackRoot::Commit(oid) => builder.insert_commit(*oid),
        }
        .map_err(|_| LocalGitFailure::Operation)?;
    }
    builder
...
fn install_packed_object_file(
    ...
    match renameat_with(
        ...
        Err(error) if error == rustix::io::Errno::EXIST => {
            unlinkat(pack_directory, &temporary_name, AtFlags::empty())
                .map_err(|_| LocalGitFailure::Operation)?;
            let existing = openat(
                pack_directory,
                name,
...
impl GitPushReceipt {
    ...
    pub fn try_new(commit: impl Into<String>) -> Result<Self, InvalidGitPushReceipt> {
        ...
        git2::Oid::from_str(&commit).map_err(|_| InvalidGitPushReceipt)?;
        Ok(Self { commit })
    }
...
impl<Transport: GitPushTransport> GitPushExecutor<Transport> {
    fn execute_push(&mut self, arguments: GitPushArguments) -> Result<String, GitPushFailure> {
        ...
        let request = GitPushRequest {
            repository_root: descriptor_path(&self.repository_authority._root),
...
fn create_loose_branch_reference(
    ...
    create_loose_branch_reference_with_hook(authority, branch, target, || {})
}
