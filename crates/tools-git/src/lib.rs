...
use rustix::fs::{
    AtFlags, CWD, Dir, Mode, OFlags, RenameFlags, mkdirat, openat, renameat_with, unlinkat,
};
use rustix::io::dup;
...
impl<FileSystem: WorkspaceFileSystem> LocalGitExecutor<FileSystem> {
    ...
    fn stage_with_pinned_objects(
        ...
        let (mut index_lock, mut index) = IndexLock::acquire(&index_path, &index_lock_path)?;
    ...
    fn branch_switch_with_hook<Hook: FnOnce()>(
        ...
        checkout_tree_with_rollback(repository, current_tree.as_ref(), &checkout_paths, || {
            repository
...
fn restore_index(
    ...
    let (mut lock, _replacement) = IndexLock::acquire(index_path, index_lock_path)?;
    lock.write(index)?;
    lock.commit()
}
...
impl IndexLock {
    fn acquire(index_path: &Path, lock_path: &Path) -> Result<(Self, Index), LocalGitFailure> {
        let lock = fs::OpenOptions::new()
            ...
            .map_err(|_| LocalGitFailure::Operation)?;
        let identity = file_identity(&lock.metadata().map_err(|_| LocalGitFailure::Operation)?);
        let private_directory = tempfile::tempdir().map_err(|_| LocalGitFailure::Operation)?;
...
mod tests {
    ...
    fn status_does_not_reclassify_a_conflict_stage_as_untracked() {
        let fixture = Fixture::new();
        install_deleted_conflict(&fixture);
        let conflict_worktree_content = b"conflict candidate\n";
    ...
    fn checkout_error_rolls_back_a_partially_written_worktree() {
        ...
        let failure =
            checkout_tree_with_rollback(&repository, Some(&current_tree), &checkout_paths, || {
                fs::write(fixture.root().join(TRACKED_PATH), CHANGED_CONTENT)
