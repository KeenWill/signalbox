...
#[tokio::test]
async fn push_ancestry_accepts_rewritten_production_lines() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let ancestor = merge_test_commit_files(&repository, &[(b"lib.rs", "fn old() {}\n")], &[]);
    let branch =
        merge_test_commit_files(&repository, &[(b"lib.rs", "fn branch() {}\n")], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"lib.rs", "fn base() {}\n")], &[ancestor]);
    let merge = merge_test_commit_files(
        &repository,
        &[(b"lib.rs", "fn combined() {}\n")],
        &[branch, base],
    );
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("content validation belongs to the required merge-ref check");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_ancestry_accepts_deleting_an_obsolete_base_fixture() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let ancestor = merge_test_commit_files(&repository, &[], &[]);
    let branch =
        merge_test_commit_files(&repository, &[(b"lib.rs", "enum Error {}\n")], &[ancestor]);
    let base = merge_test_commit_files(
        &repository,
        &[(b"tests.rs", "BlobStoreRegistryError::S3StartupDeadline,\n")],
        &[ancestor],
    );
    let merge = merge_test_commit_files(
        &repository,
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("a proven merge may remove a fixture added only by the base");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_refuses_merge_parents_that_do_not_contain_the_fence() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let ancestor = merge_test_commit(&repository, "shared\n", &[]);
    let fence = merge_test_commit(&repository, "retained\n", &[ancestor]);
    let branch = merge_test_commit(&repository, "branch\n", &[ancestor]);
