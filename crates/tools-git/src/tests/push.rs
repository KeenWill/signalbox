...
async fn push_accepts_reexpressed_branch_imports_preserving_base_lines() {
    ...
    let ancestor = merge_test_commit(
        &repository,
        "use super::{\n    GitHubToolStartup, runtime_adapter_capabilities,\n};\n",
        &[],
    );
    let branch = merge_test_commit(
        &repository,
        "use super::{\n    GitHubToolStartup, WebHttpConfiguration, ambient_database_environment,\n    runtime_adapter_capabilities,\n};\n",
        &[ancestor],
    );
    ...
    let merge = merge_test_commit(
        &repository,
        "use super::{\n    runtime_adapter_capabilities,\n};\nuse super::{\n    WebHttpConfiguration, ambient_database_environment,\n};\n",
        &[branch, base],
    );
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
...
#[tokio::test]
async fn push_accepts_regenerated_conflicting_fixtures_and_generated_markdown() {
    for (path, ancestor_text, branch_text, base_text, resolved_text) in [
        (
            b"wire_tests.rs".as_slice(),
            "    r#\"{\"kind\":\"end\"}\"#,\n",
            "    r#\"{\"kind\":\"end\",\"unavailable_component_count\":\"0\"}\"#,\n",
            "    r#\"{\"kind\":\"end\",\"session_supervision_count\":\"0\"}\"#,\n",
            "    r#\"{\"kind\":\"end\",\"unavailable_component_count\":\"0\",\"session_supervision_count\":\"0\"}\"#,\n",
        ),
        (
            b"api.md".as_slice(),
        ...
        let fixture = Fixture::new();
        let repository = Repository::open(fixture.root()).expect("repository");
        let ancestor = merge_test_commit_files(&repository, &[(path, ancestor_text)], &[]);
        let branch = merge_test_commit_files(&repository, &[(path, branch_text)], &[ancestor]);
        let base = merge_test_commit_files(&repository, &[(path, base_text)], &[ancestor]);
        let merge = merge_test_commit_files(&repository, &[(path, resolved_text)], &[branch, base]);
        let transport = RecordingPushTransport::default();
        let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
        ...
        executor
            .execute_push(GitPushArguments::for_test(FIX_BRANCH))
            .await
            .expect("regenerated conflict pushes");
...
#[tokio::test]
async fn push_regenerates_coincident_generated_counts_from_independent_parent_items() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_count = format!("{header}| types | 8 |\n");
    let parent_count = format!("{header}| types | 9 |\n");
    let merged_count = format!("{header}| types | 10 |\n");
    let branch_item = (b"branch.rs".as_slice(), "struct Branch;\n");
    let base_item = (b"base.rs".as_slice(), "struct Base;\n");
    let ancestor = merge_test_commit_files(&repository, &[(b"api.md", &ancestor_count)], &[]);
    let branch = merge_test_commit_files(
        &repository,
        &[(b"api.md", &parent_count), branch_item],
        &[ancestor],
    );
    let base = merge_test_commit_files(
        &repository,
        &[(b"api.md", &parent_count), base_item],
        &[ancestor],
    );
    let merge = merge_test_commit_files(
        &repository,
        &[(b"api.md", &merged_count), branch_item, base_item],
        &[branch, base],
    );
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("independent source additions require regenerating the coincident parent count");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_refuses_dropping_a_generated_edit_adjacent_to_the_branch_edit() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_text = format!("{header}old main\nold branch\n");
    let branch_text = format!("{header}old main\nnew branch\n");
    let base_text = format!("{header}new main\nold branch\n");
    let ancestor = merge_test_commit_files(&repository, &[(b"api.md", &ancestor_text)], &[]);
    let branch = merge_test_commit_files(&repository, &[(b"api.md", &branch_text)], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"api.md", &base_text)], &[ancestor]);
    let merge = merge_test_commit_files(&repository, &[(b"api.md", &branch_text)], &[branch, base]);
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    assert!(
        ...
        "adjacent main edit must remain verbatim: {result:?}"
    );
    assert!(!transport.has_request());
}

#[tokio::test]
async fn push_preserves_both_adjacent_generated_edits() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_text = format!("{header}old main\nold branch\n");
    let branch_text = format!("{header}old main\nnew branch\n");
    let base_text = format!("{header}new main\nold branch\n");
    let merged_text = format!("{header}new main\nnew branch\n");
    let ancestor = merge_test_commit_files(&repository, &[(b"api.md", &ancestor_text)], &[]);
    let branch = merge_test_commit_files(&repository, &[(b"api.md", &branch_text)], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"api.md", &base_text)], &[ancestor]);
    let merge = merge_test_commit_files(&repository, &[(b"api.md", &merged_text)], &[branch, base]);
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("both adjacent edits survive");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_regenerates_coincident_generated_insertions() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_text = format!("{header}anchor\n");
    let branch_text = format!("{header}| types | 9 |\nanchor\n");
    let base_text = format!("{header}| types | 10 |\nanchor\n");
    let merged_text = format!("{header}| types | 11 |\nanchor\n");
    let ancestor = merge_test_commit_files(&repository, &[(b"api.md", &ancestor_text)], &[]);
    let branch = merge_test_commit_files(&repository, &[(b"api.md", &branch_text)], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"api.md", &base_text)], &[ancestor]);
    let merge = merge_test_commit_files(&repository, &[(b"api.md", &merged_text)], &[branch, base]);
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("insertions at the same ancestor position overlap");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_regenerates_a_file_renamed_to_markdown_by_the_base() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_text = format!("{header}| types | 8 |\n");
    let branch_text = format!("{header}| types | 9 |\n");
    let base_text = format!("{header}| types | 10 |\n");
    let merged_text = format!("{header}| types | 11 |\n");
    let ancestor = merge_test_commit_files(&repository, &[(b"api.txt", &ancestor_text)], &[]);
    let branch = merge_test_commit_files(&repository, &[(b"api.txt", &branch_text)], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"api.md", &base_text)], &[ancestor]);
    let merge = merge_test_commit_files(&repository, &[(b"api.md", &merged_text)], &[branch, base]);
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("the base parent marks the renamed Markdown as generated");

    assert_eq!(transport.request().commit(), merge.to_string());
}

#[tokio::test]
async fn push_refuses_regeneration_after_the_base_renames_markdown_to_plain_text() {
    let fixture = Fixture::new();
    let repository = Repository::open(fixture.root()).expect("repository");
    let header = "<!-- generated by render.py, do not edit -->\n";
    let ancestor_text = format!("{header}| types | 8 |\n");
    ...
    let ancestor = merge_test_commit_files(&repository, &[(b"api.md", &ancestor_text)], &[]);
    let branch = merge_test_commit_files(&repository, &[(b"api.md", &branch_text)], &[ancestor]);
    let base = merge_test_commit_files(&repository, &[(b"api.txt", &base_text)], &[ancestor]);
    let merge =
        merge_test_commit_files(&repository, &[(b"api.txt", &merged_text)], &[branch, base]);
    let transport = RecordingPushTransport::default();
    let mut executor = merge_test_executor(&fixture, merge, branch, transport.clone());
    ...
    assert!(
        matches!(result, Err(GitPushFailure::MergeDroppedBaseChanges(_))),
        "a plain-text base path cannot regenerate: {result:?}"
    );
    assert!(!transport.has_request());
}

#[tokio::test]
async fn push_regeneration_exception_preserves_source_and_nonconflicting_changes() {
    for (case, path, ancestor_text, branch_text, base_text, resolved_text) in [
        (
            "production JSON is not a test fixture",
            b"wire.rs".as_slice(),
            "r#\"{}\"#,\n",
            "r#\"{\"branch\":true}\"#,\n",
            "r#\"{\"main\":true}\"#,\n",
            "r#\"{\"branch\":true,\"main\":true}\"#,\n",
        ),
        (
            "test code is not fixture data",
            b"wire_tests.rs".as_slice(),
            "assert_eq!(count, old_count);\n",
            "assert_eq!(count, branch_count);\n",
            "assert_eq!(count, main_count);\n",
            "assert_eq!(count, arbitrary_count);\n",
        ),
        (
            "fixture regeneration cannot replace the literal with code",
            b"wire_tests.rs".as_slice(),
            "r#\"{}\"#,\n",
            "r#\"{\"branch\":true}\"#,\n",
            "r#\"{\"main\":true}\"#,\n",
        ...
        (
            "fixture regeneration cannot delete the fixture",
            b"wire_tests.rs".as_slice(),
            "r#\"{}\"#,\n",
            "r#\"{\"branch\":true}\"#,\n",
            "r#\"{\"main\":true}\"#,\n",
        ...
        (
            "a branch marker does not classify the base as generated",
            b"api.md".as_slice(),
            "old\n",
            "<!-- generated by render.py, do not edit -->\nbranch\n",
            "main\n",
        ...
        (
            "generated nonconflicting spans remain verbatim",
            b"api.md".as_slice(),
            "<!-- generated by render.py, do not edit -->\n\nold\nanchor one\nanchor two\nfixed\n",
            "<!-- generated by render.py, do not edit -->\n\nbranch\nanchor one\nanchor two\nfixed\n",
            "<!-- generated by render.py, do not edit -->\n\nmain\nanchor one\nanchor two\nfixed\n",
...
async fn push_accepts_moved_base_added_lines_verbatim() {
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("line offsets do not change verbatim preservation");
...
#[tokio::test]
#[ignore = "generates five 1 GB text blobs and verifies conflict regions and base-line preservation"]
...
async fn exercise_streamed_merge(bytes: u64) {
    ...
    executor
        .execute_push(GitPushArguments::for_test(FIX_BRANCH))
        .await
        .expect("large base line survives and branch text is regrouped only inside the conflict");
    assert_eq!(transport.request().commit(), merge.to_string());
    let dropped = commit(
        "shared\nbase conflict\nbranch regrouped\nanchor one\nanchor two\n",
        "ancestor\n",
