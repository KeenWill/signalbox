...
pub(super) fn verify_merge(
    ...
    for delta in carried.deltas() {
        ...
        let base_path = versions[2].path().unwrap_or(source_path);
        if (streamed::regeneratable_path(source_path) || streamed::regeneratable_path(base_path))
            && let Some(protected) = contents.protected_hunks(
                source_path,
                base_path,
                versions,
                [&branch_hunks, &base_hunks, &result_hunks],
            )?
        {
            base_hunks = protected;
        }
...
impl<'a> Contents<'a> {
    ...
    fn protected_hunks(
        ...
        versions: [Side<'_>; 4],
        hunks: [&Hunks; 3],
    ) -> Result<Option<Hunks>, GitPushFailure> {
        if hunks.iter().any(|hunk| hunk.opaque) || hunks[1].edits.is_empty() {
            return Ok(None);
        }
        let [Some(ancestor), Some(branch), Some(base), Some(result)] = [
            self.lines(versions[0])?,
            self.lines(versions[1])?,
            self.lines(versions[2])?,
            self.lines(versions[3])?,
        ] else {
            return Ok(None);
        };
        streamed::protected_base_hunks(
            path,
            base_path,
            [&ancestor, &branch, &base, &result],
            hunks,
            self.deadline,
        )
        .map(Some)
    ...
    fn preserves(
        ...
        streamed::preserves_indexed_text(
            [&ancestor, &branch_lines, &base_lines],
            base_side.path().unwrap_or(Path::new("")),
            &result,
            branch,
            base,
            self.deadline,
