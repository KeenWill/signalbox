...
/// An OpenAI API key value.
///
/// Debug output is redacted and the value is only ever attached to one
/// request as a sensitivity-marked `Authorization` header; this crate never
/// logs (ADR-0017's credential boundary).
#[derive(Clone)]
...
pub struct OpenAiConfig {
    /// Source of the `Authorization: Bearer …` value, read once per
    /// operation during send preparation (ADR-0017).
