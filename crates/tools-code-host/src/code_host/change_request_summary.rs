//! `change_request_summary` registry declaration and typed arguments.
...
pub(super) const NAME: &str = "change_request_summary";
pub(super) const DESCRIPTION: &str =
    "Returns bounded summary metadata for one GitHub change request.";

#[derive(Clone, Debug, Eq, PartialEq, serde::Deserialize, schemars::JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct ChangeRequestSummaryArguments {
...
impl ToolContract for Contract {
    type Arguments = ChangeRequestSummaryArguments;
...
pub(super) fn decode(
    ...
    decode_arguments(arguments).map(CodeHostOperation::Summary)
}
