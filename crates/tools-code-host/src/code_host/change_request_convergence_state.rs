//! `change_request_convergence_state` registry declaration and typed arguments.
...
pub(super) const NAME: &str = "change_request_convergence_state";
pub(super) const DESCRIPTION: &str = "Returns bounded convergence evidence and the deterministic verdict for one GitHub change request.";

#[derive(Clone, Debug, Eq, PartialEq, serde::Deserialize, schemars::JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct ConvergenceStateArguments {
...
impl ToolContract for Contract {
    type Arguments = ConvergenceStateArguments;
...
pub(super) fn decode(
    ...
    decode_arguments(arguments).map(CodeHostOperation::ConvergenceState)
}
