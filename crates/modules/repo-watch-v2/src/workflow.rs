...
impl RepoWatchStore {
    ...
    >(
        ...
        let next: Option<(Decimal, Uuid, Vec<u8>)> = sqlx::query_as("SELECT event.repository_event_ordinal, event.event_id, event.normalized_payload FROM rule active JOIN rule_revision revision ON revision.repository = active.repository AND revision.rule_id = active.rule_id AND revision.revision = active.active_revision LEFT JOIN rule_evaluation_cursor cursor ON cursor.repository = active.repository AND cursor.rule_id = active.rule_id AND cursor.rule_revision = active.active_revision JOIN gh_readable_event event ON event.repository = active.repository AND event.repository_event_ordinal > GREATEST(revision.activated_after_event_ordinal, COALESCE(cursor.event_ordinal, 0)) WHERE active.repository = $1 AND active.rule_id = $2 AND active.active_revision = $3 AND revision.content_digest = $4 AND cursor.effect_id IS NULL AND NOT EXISTS (SELECT 1 FROM observer_actor actor WHERE actor.repository = active.repository AND NOT actor.ready) ORDER BY event.repository_event_ordinal LIMIT 1")
