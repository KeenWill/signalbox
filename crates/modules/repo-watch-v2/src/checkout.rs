...
impl RepoWatchStore {
    ...
    pub async fn dispatch_checkout(
        ...
        let row: Option<CheckoutRow> = sqlx::query_as(
            "SELECT ledger.dispatch_ref, event.event_id, COALESCE(ledger.activation_event, ledger.retry_event, event.normalized_payload) AS normalized_payload, ledger.checkout_head_sha, ledger.checkout_removed, ledger.checkout_stop_command_id, ledger.checkout_retired_reason, ledger.checkout_workspace_root, ledger.checkout_session_id
