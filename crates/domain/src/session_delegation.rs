...
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
enum ParentTerminationOutcomeKind {
    ContinueRunning,
    ChildStopped,
    ChildCancelled,
}

/// Complete stored record for replaying one parent termination outcome.
///
/// This input does not expose parent termination authority. A relation can
/// consume it only through its checked reconstitution method, which binds the
/// exact parent, spawning turn, command, termination kind, scope, and recorded
/// child action before recreating the sealed authority internally.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct ParentTerminationOutcomeReconstitutionInput {
    parent: SessionId,
    turn: TurnId,
    command: DurableCommandId,
    kind: ParentTerminationKind,
    scope: DescendantTerminationScope,
    outcome: ParentTerminationOutcomeKind,
}
...
impl ParentTerminationOutcomeReconstitutionInput {
    pub const fn new(
        ...
        let outcome = match action {
            BoundChildAction::KeepRunning => ParentTerminationOutcomeKind::ContinueRunning,
            BoundChildAction::Stop => ParentTerminationOutcomeKind::ChildStopped,
            BoundChildAction::Cancel => ParentTerminationOutcomeKind::ChildCancelled,
        };
        Self {
            parent,
            turn,
...
impl SessionDelegation {
    ...
    /// Replays one complete parent termination record without exposing raw-ID
    /// construction of [`ParentTerminationAuthority`].
    pub fn reconstitute_parent_termination_outcome(
        self,
        input: ParentTerminationOutcomeReconstitutionInput,
    ) -> Result<Self, DelegationTransitionError> {
        let Some(spawning_turn) = self.events.first().and_then(|event| match event {
            DelegationEvent::Spawned { provenance, .. } => {
                provenance.tool_request().map(|(_, turn, _)| turn)
            }
            DelegationEvent::MessageDelivered { .. } | DelegationEvent::OutcomeRecorded { .. } => {
                None
            }
        }) else {
            return Err(self.fail(DelegationTransitionFailure::MissingSpawnEvent));
        };
        if input.parent != self.parent || input.turn != spawning_turn {
            return Err(self.fail(DelegationTransitionFailure::InvalidProvenance));
        }
        let reason = match input.kind {
            ParentTerminationKind::Stopped => {
                DelegationOutcomeReason::ParentStopped { scope: input.scope }
            }
            ParentTerminationKind::Cancelled => {
                DelegationOutcomeReason::ParentCancelled { scope: input.scope }
            }
        };
        let provenance =
        ...
        let outcome = match input.outcome {
            ParentTerminationOutcomeKind::ContinueRunning => {
                DelegationOutcome::ContinueRunning { reason, provenance }
            }
            ParentTerminationOutcomeKind::ChildStopped => {
                DelegationOutcome::ChildStopped { reason, provenance }
            }
            ParentTerminationOutcomeKind::ChildCancelled => {
