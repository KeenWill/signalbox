...
- Construct pending steering from exactly one `SteeringBinding`; prove consumption validates the referenced call belongs to that source turn and derives the immutable frontier from the call. No domain constructor accepts a duplicate turn or frontier that could be cross-wired.
