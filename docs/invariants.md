...
| INV-008 | Accepted logical work has a durable immutable effective-configuration reference sufficient to explain later model and policy choices. Every field in the baseline typed value is identity-significant. Origin turns create it atomically with accepted input; safe-point input inherits and captures the active turn's value as fallback. | Transaction-level | Accepted provenance; freeze and equality boundaries proposed in ADR-0027 | Create logical work and configuration snapshot/reference atomically; compare the complete typed value; never reread mutable defaults as accepted intent. |
| INV-009 | At most one logical turn actively progresses in a session; every proposed `Active` phase, including durable waits and cancellation, retains the slot. `Active(Running)` has exactly one current nonterminal turn attempt created atomically with activation or wait resolution; waiting phases have none. | Database-level | Single-turn rule accepted; exact active-state and attempt membership proposed in ADR-0004 | Database uniqueness/exclusion or serialized transition, not process memory alone. |
...
| ID | Invariant | Class | Status | Expected enforcement |
| --- | --- | --- | --- | --- |
| INV-014 | Every hub authorization to attempt a provider interaction has a model-call identity and records the requested model selection and exact hub-resolved provider/model target, even if it fails before send; provider-reported or otherwise observable identity and mismatch are appended when available. This provenance does not claim knowledge of an undisclosed physical backend. | Transaction-level | Requested/resolved/reported provenance accepted; pre-send identity and retry boundary proposed in ADR-0001 and ADR-0005 | Persist call, request, and hub resolution before sending, then append observable provider identity and mismatch evidence when available. |
...
| INV-016 | Safe-point steering never mutates an already-issued provider request, tool request, approval, or tool attempt, and accepted steering never disappears without a durable disposition. The baseline safe point requires every earlier issued physical operation for the turn to be classified. Steering inherits the turn's configuration; once consumed, it is committed turn history and remains in later retry or continuation frontiers. | Runtime validation | No-mutation/no-loss direction accepted; provider-call-only safe point, classified-operation guard, and reclassification proposed in ADR-0027 | Consume steering atomically at the allowed boundary, preserve it in later frontiers, or reclassify it visibly when the target turn terminates before consumption. |
...
| INV-029 | Interrupt stops future progress as promptly as safely possible, retains the predecessor's progressing slot through terminal classification of every issued operation, and never claims to undo an already-issued external effect. An ambiguous issued effect terminalizes the interrupted turn as reconciliation required. | Operational policy | No-rollback direction accepted; terminal guard, slot, and successor behavior proposed in ADR-0004 and ADR-0027 | Cancellation signals plus honest outcome classification; replacement work waits for predecessor terminal disposition. |
...
| INV-034 | Recovery ends or fences every nonterminal pre-restart attempt and classifies interrupted physical work as completed, known failed/lost, cancelled, ambiguous, or requiring reconciliation; it does not silently discard it or reopen terminal work. A non-cancelled ambiguity retains the active slot in `AwaitingRecoveryDecision`; cancellation plus ambiguity terminalizes for reconciliation. | Runtime validation | Accepted classification direction; exact attempt and ambiguity dispositions proposed in ADR-0004 and ADR-0005 | Startup reconciliation scans, explicit durable transitions, and fenced replacement attempts/calls. |
