...
| INV-013 | Archiving changes discoverability/lifecycle state but does not erase durable history; restoration preserves identity. | Database-level | Provisional | Archive state transition; destructive retention is a separate future policy. |

## Model execution

| ID | Invariant | Class | Status | Expected enforcement |
| --- | --- | --- | --- | --- |
| INV-014 | Every initiated provider interaction records its exact resolved provider/model reference; requested alias alone is insufficient history. | Transaction-level | Accepted | Persist call and resolution before sending, then append provider-observed substitution when available. |
...
| INV-016 | Safe-point steering never mutates an already-running provider request. | Runtime validation | Accepted | Eligibility is evaluated only at orchestration boundaries before another model call. |
...
| INV-022 | A runner's advertised execution boundary must truthfully reflect its actual deployment; absence of strong isolation is represented explicitly. | Operational policy | Accepted; attestation mechanism open | Deployment configuration and registration validation; visible snapshots when work is assigned. |
| INV-023 | Selecting an ambient-user runner is explicit and inspectable; the system does not label it sandboxed. | Runtime validation | Accepted | Capability matching, UI disclosure, and durable execution-boundary snapshot. |
