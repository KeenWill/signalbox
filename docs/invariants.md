...
| INV-006 | Each transition is valid only from explicitly permitted prior states; terminal turns and physical outcomes do not return to running. | Domain/type-level | Accepted in principle; exact turn/attempt state sets proposed in ADR-0004 | Total transition functions over typed states plus persistence guards. |
...
| INV-015 | Every model call records the exact context frontier it consumed; a turn is not assumed to have one immutable frontier. | Transaction-level | Accepted | Freeze and persist each call frontier before issuing the provider request. |
| INV-016 | Safe-point steering never mutates an already-issued provider request, tool request, approval, or tool attempt, and accepted steering never disappears without a durable disposition. | Runtime validation | No-mutation/no-loss direction accepted; provider-call-only safe point and reclassification proposed in ADR-0027 | Consume steering atomically at an allowed future boundary or reclassify it visibly when the target turn terminates. |
...
| INV-029 | Interrupt stops future progress as promptly as safely possible, retains the predecessor's progressing slot through terminal classification, and never claims to undo an already-issued external effect. | Operational policy | No-rollback direction accepted; slot and successor behavior proposed in ADR-0004 and ADR-0027 | Cancellation signals plus honest outcome classification; replacement work waits for predecessor terminal disposition. |
