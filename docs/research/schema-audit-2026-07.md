...
| `turn_lifecycle`                                | `180004`, `220001`, `220003`, `220004`, `220005`, `240003`, `250001` | 7/13/1/4/3 | `turn_lifecycle_by_session_position`; `turn_lifecycle_one_active_per_session`; `turn_lifecycle_queued_by_session` |        4 | Inline the final lifecycle/tool-loop validators; review workflow will reshape it again   |
...
- The live `require_submit_input_legacy_effect_correlation` name remains the
  ...
  `require_non_interrupt_submit_input_effect_correlation`, with both trigger
  references and the one name-pinning test updated.
...
**Risk.** Trigger definitions and a function-name-pinning integration test must
move atomically. There is no stored-data risk.
...
- [persistence-protocol](../spec/persistence-protocol.md),
  ...
  old file counts, versions, or succession.
...
7. Apply the baseline to a second empty database. Diff normalized catalogs
   ...
   removal and physical-name changes. Confirm all 58 checkpoint tables and the
   logical frontier view are present.
...
lands and the owner declares the checkpoint**. Its implementation should be a
full two-phase baseline reset. The final schema needs only two small
correct-choice cleanups—one duplicate key/FK pair and consistent delta names—
but the migration set has enough interleaved history that a partial squash would
preserve the tax without materially lowering execution risk.
