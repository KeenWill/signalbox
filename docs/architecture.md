...
3. No-active-turn, interrupt, and after-current input create origin turns and freeze their complete effective configuration at acceptance. Every field in that value is identity-significant. Safe-point input inherits the active turn's configuration and captures it as fallback in case the input must be reclassified. These typed delivery transitions determine turn identity; the hub does not compare natural-language intent to choose an identity.
4. Queue eligibility is derived from durable lineage and slot ownership. After its immediate predecessor becomes terminal, a queued or interrupt-created turn atomically fixes its outcome-aware starting context frontier and either activates with an initial physical attempt or fails with that complete frontier. It cannot terminalize ahead of its predecessor.
