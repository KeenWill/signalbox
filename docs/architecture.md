...
3. Durable-command deduplication first looks up one owner-global identifier across every discriminated command kind, session, and client. The same typed variant and caller-supplied fields return the stored terminal applied-or-rejected result; reuse for another variant, session, or payload is rejected before current-state validation or configuration derivation. For an unseen well-formed typed command under established owner authority, the hub validates authoritative session state and the complete versioned resolver read set, derives configuration, and atomically records the comparison payload plus either a typed rejection or the applied result. Application makes the accepted-input identity, content, explicit treatment, ordering, initial disposition, and configuration provenance durable before acknowledgement. A recorded rejection never revalidates after state changes; corrected intent uses a new identifier. Malformed transport, pre-authority, and pre-commit infrastructure failures that do not reach committed domain handling claim no identifier.
