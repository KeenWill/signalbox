...
Accepted [ADR-0027](decisions/0027-input-delivery-lifecycle.md) defines the durability flow as follows:

1. Session creation establishes an immutable first version of user/session-level model-selection defaults. Explicit defaults updates create later versions and affect only origin input accepted afterward; they never rewrite queued or active work.
...
3. After transport decoding, owner-scope establishment, and purpose-specific canonical construction, durable-command deduplication first looks up one owner-global identifier across every discriminated command kind, session, and client. Equivalent boundary forms normalize to the same typed caller payload. A form that cannot construct that payload claims no identifier and may be corrected under the same one. The same constructed variant and canonical caller fields return the stored terminal applied-or-rejected result; reuse of a claimed identifier for another variant, session, or payload is rejected before current-state validation or configuration derivation. For an unseen well-formed typed command under established owner authority, the hub validates authoritative session state and the complete versioned resolver read set, derives configuration, and atomically records the comparison payload plus either a typed rejection or the applied result. Application makes the accepted-input identity, content, explicit treatment, ordering, initial disposition, and configuration provenance durable before acknowledgement. A recorded rejection never revalidates after state changes; corrected domain intent uses a new identifier. Malformed transport, pre-authority, unconstructible-command, and pre-commit infrastructure failures that do not reach committed domain handling claim no identifier. A raw command identifier or rejection never supplies lifecycle authority: interrupt cancellation and owner-chosen reconciliation carry purpose-specific proofs constructible only from matching applied command results.
4. No-active-turn, interrupt, and after-current input create origin turns and freeze their complete baseline effective configuration at acceptance. The first constructible algebra contains one canonical direct selection or frozen alias selecting one direct choice, provider defaults, and disabled known-failure retry/fallback. Custom parameters, instructions, tools, placement, per-turn resources, and interpreting-policy choices remain unavailable until their subsystem ADRs extend request, defaults, overrides, and effective values together. Operational connection, scheduling, credential, telemetry, and transport-timing facts remain outside. Safe-point input captures a reference to the source turn as inherited reclassification provenance rather than copying its immutable configuration or inventing a request. These typed delivery transitions determine turn identity; the hub does not compare natural-language intent to choose an identity.
...
Acknowledged work must not vanish. On restart, a startup scan reconstructs accepted-input dispositions, queued work with frozen configuration provenance, confirmation waits, any future accepted delegation waits, interrupted attempts, and already-applied interrupt proofs from Postgres. It ends each nonterminal turn attempt owned by the prior process with disposition `Lost`; `Lost` describes the abandoned orchestration tenure, not a model-call or generic physical-operation outcome, while the matching terminal variant retains any already-applied interrupt and every fatal cause already durable or first established by the scan. The scan is not itself an attempt and cannot issue semantic effects. In one serialized transition it derives the complete recovered evidence set before classifying each interrupted physical operation as completed, known failed, refused where applicable, cancelled, or ambiguous and selecting turn precedence. Live and startup classification share one precedence: unacknowledged ambiguity first (wait, or proof-bearing reconciliation with interrupt/fatal stop), then sufficient outcome-authoritative non-mismatched completion/refusal only without fatal stop, known failure or mismatch invalidation, and applied-and-confirmed interrupt cancellation. A physical cancellation without the exact applied interrupt proof remains honest operation evidence but supplies turn failure rather than fabricating a cancelled turn or unstopped cancelled attempt. A completion or refusal raced under fatal stop remains honest physical evidence but cannot control the turn. Mismatch never reopens a terminal call: after ambiguity it supports turn failure when no other ambiguity remains and reconciliation otherwise; after completion during an active turn it appends typed invalidation and stops future effects; after terminal known failure/cancellation or an atomically refused turn it preserves that state and precedence. `ReconciliationRequired` is a turn disposition carrying the exact canonical nonempty unacknowledged ambiguity set plus an applied owner-choice, interrupt, or fatal-mismatch reason; it never replaces the physical operation's `Ambiguous` classification. Under accepted ADR-0004, ambiguity retains the turn's active slot in `AwaitingRecoveryDecision` only when neither applied interrupt nor fatal mismatch prohibits continuation; owner-authorized continuation preserves the physical ambiguity and records `DuplicateRiskAccepted`, which later turn outcomes and successor context retain. Startup creates no cancellation-only or classification-only replacement attempt.

At most one logical turn actively progresses per session initially. Accepted [ADR-0004](decisions/0004-turn-and-attempt-lifecycle.md) defines every implemented active phase, including approval and recovery waits, as retaining the session slot. The turn aggregate owns exactly one current attempt whose `Prepared`, `Running`, or `StopRequested` variant is its single nonterminal state; a stop request is typed as interrupt-only with its applied proof or fatal mismatch with a nonempty failure set and optional applied interrupt, survives restart, and prohibits new semantic effects. A `Running` attempt with a still-unresolved physically `Ambiguous` owned operation is likewise classification-only under an aggregate dispatch guard. Nonfatal evidence may clear that blocking condition without reopening the physical record; the same attempt may resume only after every other issued operation is classified. Fatal evidence instead enters or extends the typed stop state. Fatal-stop terminal history cannot represent completion, refusal, or cancellation. Persistence may still use separate records. Applying an interrupt to a prepared attempt ends it with the turn. A durable wait carries its exact subject and no attempt. Every operation already issued by an attempt must be terminally classified before that attempt yields to any wait. An interrupt from a wait closes that wait and terminalizes atomically with the matching proof-bearing disposition. No standalone active-turn cancellation exists in the baseline. No turn terminalizes or releases its slot until every owned logical tool/approval dependency is closed or terminally non-dispatchable, every issued physical operation is classified, its attempt is ended, and any wait is closed. Future child waits require ADR-0002 and will retain the slot unless that ADR defines explicit branching semantics. Scheduler mechanics remain open.
