...
The process protocol carries a single integer **protocol version**. A version names the complete exchange semantics: ADR-0019's request families, envelope, rejection classes, and event typing, the message schema baseline, and the negotiation *rules* this record applies within that version — the versions offered, the capabilities defined, and the window promise. The one thing a version never names is the negotiation offer-and-result *frame* itself: that frame is version-independent by construction (see the handshake below), so two speakers sharing no protocol version can still exchange a typed refusal. A future version may change the negotiation rules that run over that frame, but never the frame that carries them. Version one is `1`. Anything that removes or changes the meaning of existing protocol content increments the version; a version is a statement about meaning, not a build number, and two releases that interoperate silently under the same version must mean the same thing by every exchange.

### Capabilities gate additions within a version

A **capability** is a named, documented, additive unit of optional protocol content — a new command kind, snapshot projection, transient (draft or progress) event type, or field group — that a version-`n` speaker may support without a new version. Capabilities are only ever additive: removing or changing the meaning of anything, including a capability itself, is a version increment, not a capability change. A capability never gates a **durable-transition (cursor-advancing) event**: ADR-0019 forbids a subscription from skipping a committed cursor, so every durable transition must be representable to any client that negotiated the version — adding a new cursor-advancing event is therefore a version increment, never a capability, and capabilities gate only content that does not by itself advance the cursor. Version one ships the mechanism with an **empty capability set**: everything in version one is required, and the first real capability arrives with whatever slice needs one.

### The handshake and the sending discipline

Negotiation is semantic, and its rules bind every transport mapping:

- A client presents the protocol versions it supports and the capabilities it requests. The hub answers with the negotiated version — the highest version both support — and the granted capability set (the intersection of requested and hub-supported), or with the typed incompatibility below. An explicit negotiation exchange is available to clients as its own unary request, and every request is attributable to one negotiated version and capability set; whether that attribution is carried per connection, per request, or both is fixed with the transport slice under ADR-0019. The negotiation exchange itself — the version-and-capability offer and its typed result, including the incompatibility below — is version-independent: it is a stable frame that is never subject to the version increment, so every hub and every client can decode it regardless of which protocol versions they otherwise share. That is what makes an explicit typed refusal possible even when no protocol version is common; ADR-0019's transport slice pins the concrete shape of this stable frame.
...
- **The hub never sends content outside the negotiated version and capability set** — no event kind, message, field group, or variant the negotiation did not establish. Under that discipline, unknown required content received by either side is a defect, and the rule for it is INV-033's: fail explicitly, never guess. A client that cannot interpret a durable-transition event does not skip it; it surfaces the failure and may re-snapshot only after renegotiating a compatible baseline. This discipline requires that silent tolerance of unknown wire content be impossible: every schema and transport mapping must surface content outside the negotiated set as an explicit error rather than discard or preserve it unread — the default proto3 behavior of accepting and ignoring unknown fields is not acceptable on any mapping (ADR-0019's schema selection), so capability-gated content is carried behind negotiated discriminators an older speaker rejects, never as bare optional fields it silently drops.
...
- Clients presenting only versions older than the window receive the typed incompatibility with upgrade guidance — an explicit refusal, never degraded or guessed operation. This is deliverable precisely because the negotiation frame above is version-independent: an out-of-window client can still decode the refusal even though it shares no protocol version with the hub.
...
- The runner boundary's version line, capabilities, and any justified divergence belong to the reserved runner record (ADR-0008).
