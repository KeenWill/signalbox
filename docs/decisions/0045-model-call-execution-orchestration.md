...
the fatal handoff is a live reconciliation obligation, not a redispatch. Hubd
supervision retains the call identity and in-memory observation while it
requests cancellation and drains the still-running provider interaction. A
successful drain hands the unchanged observation to a later authoritative pass;
the provider is never invoked again for that call. If the process cannot drain
or stops first, the retained observation is advisory only and ADR-0004 startup
recovery rereads durable state and provider evidence to classify the issued call
and release the session slot. This is the only path that may retain an
uncommitted observation after provider work has begun.
...
- Per application invocation, the service invokes the provider interaction at
  ...
  durable inputs. An authorize-send identity-collision rollback preserves its
  already prepared, unconsumed capability while retrying with fresh candidates.
  Prepare retries have no capability; observation retries preserve the unchanged
  in-memory observation after the capability was consumed. None repeats
  credential I/O or provider work.
