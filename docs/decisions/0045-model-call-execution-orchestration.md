...
08. if credential lookup or local capability preparation returned a trustworthy
    ...
    candidates without repeating capability preparation;
09. only after capability preparation succeeds, acquires the process-shared
    attempt dispatch/stop gate, then opens a distinct authorize-send transaction
    that reloads authority and either rejects stale or stopped work, or commits
    `Prepared -> InFlight` with its attempt, lifecycle, and outbox facts; and
...
pauses provider I/O before invoking the transaction port, awaits the immediate
commit of that observation and its durable best-effort cancellation request,
then resumes provider processing within the same provider-port invocation. No
provider read, stream advance, or other interaction processing continues while
durable ordering is unresolved. After a successful commit the port continues
only to observe that already-started interaction and may return its later
physical outcome or audit evidence; this is neither a second provider
...
| The process stops after committing `Prepared` but before same-incarnation resume                                                  | Startup proves no send authorization or provider acceptance existed, terminalizes the call `KnownFailed`, and ends the prior-process attempt `Lost` in ADR-0004's single recovery transition. It derives the turn result from the complete evidence set; it never resumes that attempt or dispatches the call.                                                                                               |
...
| A no-call, `Prepared`, or known-failure prepare commit is ambiguous                                                               | Return a prepare-stage commit-ambiguous failure, invoke no later port, and reread authoritative state before any later action.                                                                                                                                                                                                                                                                               |
| The authorize-send commit is ambiguous before provider invocation                                                                 | Do not invoke the provider. Preserve proof that the capability was never consumed, discard it, and reread authoritative state. A proven rollback leaves `Prepared` for a later pass; if `InFlight` committed, submit that non-consumption proof as ADR-0043 known-failure evidence and atomically classify the call and owning lifecycle without sending.                                                    |
...
| An early streaming-observation commit is ambiguous or rolls back for any reason other than a proven hub-minted identity collision | Report failure to the paused provider control path, request in-process best-effort cancellation, and retain ownership until the interaction stops or is drained. A successful drain transfers the unchanged observation to a same-incarnation authoritative pass. If draining fails or the provider/process is lost, escalate fatally so startup recovery rereads authoritative state and provider evidence. |
...
| A terminal-observation commit has any other failure, including commit ambiguity                                                   | Return the commit-stage failure and retain the unchanged terminal observation for an authoritative reread. Do not invoke the provider again. If reread proves the observation absent after rollback, a later pass may commit that retained observation; if it proves the first commit landed, discard the retained copy. Process loss falls back to startup recovery from durable and provider evidence.     |
...
When that early observation commit fails outside the identity-collision retry,
the live reconciliation obligation is not a redispatch. Hubd supervision retains
the call identity and in-memory observation while it requests cancellation and
drains the paused provider interaction. A successful drain hands the unchanged
observation to a same-incarnation authoritative pass without fatal escalation;
the provider is never invoked again for that call. If the process cannot drain
or stops first, the retained observation is advisory only and fatal escalation
lets ADR-0004 startup recovery reread durable state and provider evidence to
classify the issued call and release the session slot.

A terminal observation likewise remains owned in memory after a failed commit.
Because the provider interaction has already ended, the service first rereads
authoritative state: a known rollback or proven absence permits a later pass to
commit the unchanged observation, while evidence that the first commit landed
discards the retained copy. Commit ambiguity never authorizes blind replay, and
process loss leaves startup recovery to classify only from durable and provider
evidence. These early-drain and terminal-reread paths are the only paths that
may retain an uncommitted observation after provider work has begun.
...
- Per application invocation, the service invokes the provider interaction at
  ...
  durable inputs. A non-collision observation failure is not retried blindly in
  the same pass; the unchanged observation may be submitted by a later pass only
  after authoritative reread proves it absent. An authorize-send
  identity-collision rollback preserves its already prepared, unconsumed
  capability while retrying with fresh candidates. Prepare retries have no
  capability; observation retries preserve the unchanged in-memory observation
  after the capability was consumed. None repeats credential I/O or provider
  work.
