...
Signalbox persistence uses a **Postgres-native, normalized relational schema of purpose-specific storage records with explicit, versioned, in-repo migrations**. Lifecycle aggregates are current-state rows carrying typed state discriminators and constraint-enforced exclusivity. Append-only tables are used exactly where an accepted ADR already defines the fact as immutable once committed — the durable-command registry, session creation provenance, session-defaults versions, accepted-input content and order facts, ended turn attempts, provider target evidence and invalidations, outcome-authority transfers, ambiguity acknowledgements, and semantic history entries. There is no event store from which current state is replayed: the guarded row is the durable statement of record, which keeps INV-009 and INV-012 enforceable as declarative constraints rather than projection code.
...
- Ordinal values such as `SessionInputPosition` and `SessionConfigurationDefaultsVersion` are `u64` domain ordinals starting at one. Postgres `bigint` is signed; the mapping stores them as `bigint`, rejects non-positive values on load, and treats overflow at the boundary explicitly rather than reinterpreting the domain type.
...
| Session creation provenance (ADR-0003) | `session`: `session_id`, creation-cause discriminator (baseline `owner_initiated` only, `CHECK`-closed), ancestry columns (`source_session_id`, source-frontier reference) constrained both-null or both-present. Immutable after insert. |
| Session model-selection defaults (ADR-0027; `VersionedSessionConfigurationDefaults`) | Append-only `session_defaults_version`: `(session_id, version)` primary key plus the complete normalized `ModelSelectionRequest` columns. The current version is referenced from `session` and advanced only by compare-and-set against the caller's expected version. |
