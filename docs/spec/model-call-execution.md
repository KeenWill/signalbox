...
details remain observation evidence. During automatic compaction, when framing
or an indivisible exchange exceeds the compaction input budget, source text is
bounded with UTF-8-safe truncation and retained/dropped byte markers. The
complete serialized request is measured when bounding that material. The
automatic call summarizes that bounded material. Its summary text is bounded by
