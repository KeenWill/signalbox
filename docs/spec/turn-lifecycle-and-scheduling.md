...
physically in flight. The child is no longer scheduler work; physical
cancellation latency cannot revive it, replace its result, or change the typed
stop/cancel provenance. A background or bound `KeepRunning` edge has no such
proof and remains schedulable. This proposal is accepted with the implementing
stack's merge.
