...
- `Interrupt` targeting the active turn atomically accepts a configured
  ...
  follow [model-call-execution](model-call-execution.md). A matching interrupt
  against `AwaitingRecoveryDecision` preserves the already terminal ambiguous
  call and ended attempt, records the new proof on the turn's reconciliation
  marker, and terminalizes `ReconciliationRequired` with the wait's exact
  operation set. The `reconcile_turn` request in
