...
Its version-one JSON corpus carries each synthetic tool request and frozen
authority context; valid JSON argument text is normalized by the daemon renderer
before judging. It also carries an expected `approve`, `deny`, or
`escalate_to_human` disposition and nonempty free-text label provenance. Its
