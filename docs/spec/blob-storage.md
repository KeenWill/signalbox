...
scheduler-pass capacity. Because each task relinquishes its scheduler slot during
store traversal, slow reads cannot occupy every configured scheduler-pass slot,
and the fixed direct-read bound keeps the handoff's waiter inventory bounded.
