...
publication. Loose-object, packed-base, and packed-delta decompression check
preparation deadlines on each bounded compressed-input read, including blocks
that produce no output. Private-pack writes check preparation deadlines between
fixed-size pages. Unsupported layouts and formats, exhausted bounds, allocation
