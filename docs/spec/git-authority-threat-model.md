...
snapshot and streamed content digest, and new files are created exclusively.
Copied files retain descriptor ownership and pass pathname, identity, and
target-blob hash checks; captured checkout content must match the target before
index publication. Merge verification retains bounded previews and uses
file-backed comparison scratch data with linear-space divide-and-conquer line
matching; disjoint replacements use a linear scan. A fixed budget bounds
