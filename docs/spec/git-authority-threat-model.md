...
publication streams each batch into one pack and index pair. Merge verification
retains bounded previews and uses file-backed comparison scratch data with
linear-space divide-and-conquer line matching; disjoint replacements use a
linear scan. Private-pack writes and merge comparison check preparation
