...
over-ceiling reference derives unavailable configuration. A timestamp no
configured window covers yields no dollar figure, never zero. Independently
...
window that priced it and its `real` or `metered_equivalent` label. Unavailable
cost carries one closed reason: no token evidence, unknown input semantics,
incomplete cache axes, invalid cache breakdown, no covering rate window, or
