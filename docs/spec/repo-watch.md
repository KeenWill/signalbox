...
transit is resolved by reading whether the row is already terminal, which cannot
itself be ambiguous: if it is, the delivery counts as recorded and the shadow
advances; if it is not, the record is re-attempted a bounded number of times
before the delivery is left pending for the next drain. If every settling read
is itself unavailable, the shadow is discarded rather than trusted, because a
disposition may have landed without being reflected in that baseline. A durable
disposition the shadow never accounted for is what this avoids. A delivery whose
