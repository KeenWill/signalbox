...
watched pull requests whose provider events stopped arriving. It uses `signalbox-convergence` for its predicate and owns its fenced
commission, durable retry and park records, and configuration throttle. It is opt-in twice:
...
`convergence_pull_requests`. The `[convergence]` policy supplies the shared predicate described in
[review workflows](review-workflows.md). The sweep evaluates one revalidated
snapshot and uses that verdict for its decision.
