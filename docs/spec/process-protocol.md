...
(`agent/turn-reconciliation-recovery`). Eight was taken while seven was reserved
by then-open PR #281; #281 merged before this branch began. Eight remains taken
by then-open PR #291 (`agent/turn-control-verbs`), and nine remains taken by
then-open PR #286 (`agent/session-system-prompt`); this imported-continuation
stack therefore takes version ten. The implementation here speaks versions one
through seven and ten while eight and nine remain unsupported, and its terminal
client selects version ten. This page is the normative boundary between a local
...
- `version`: JSON integer `1`, `2`, `3`, `4`, `5`, `6`, `7`, or `10`;
...
spellings decode to the same name. A version other than one through seven or ten
...
the pre-admission fallback. A client speaking version two through seven or ten
...
adds only `reconcile_turn`. Versions eight and nine are reserved and unsupported
in this stack. Version ten retains every version-one-through-seven request and
...
five, a reconciliation request carried under version one through six, or an
imported-frontier creation request carried under any version one through seven,
...
Versions four through seven and ten also inherit every transcript, turn-state,
...
introduced by version three. A version-four through version-seven or version-ten
...
`replace_session_metadata`, `replace_session_defaults`, and `reconcile_turn`, a
...
`reconcile_turn` retry reaches that recorded outcome unconditionally, because a
claimed command identity bypasses the precondition the first handling already
satisfied. A definitely pre-commit infrastructure failure maps to `unavailable`.
...
All admitted versions forward durable transition events only. Provider token
...
The `signalbox` binary in this stack uses version ten; version four's metadata
...
the requested selection and copied posture, renders only known version-ten
