...
- `version`: JSON integer `1` through `11`, or `13`;
...
thirteen produces an `unsupported_version` error naming the supported versions,
...
twelve is reserved and unsupported. Version thirteen retains every earlier
admitted request and adds only the optional delivery member on `submit_input`
plus its steering receipt. A metadata request carried under version one, two, or
...
through ten, or a delivery-bearing submit carried under any version before
thirteen, is classified as `malformed_frame` because its supported version does
