...
All six versions use one Unix domain stream socket. The hub requires its path in
...
The `signalbox` binary in this stack uses version six. Older clients remain
supported for representations admitted by their declared version as described
above. The client accepts a global `--socket <path>` override or reads
