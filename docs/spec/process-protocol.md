...
(`agent/token-level-streaming`). The implementation speaks versions one through
twelve, and its terminal client selects version twelve. Its `search` verb over
...
- `version`: JSON integer `1` through `12`;
...
twelve retains every earlier admitted request and adds no request variant. A
