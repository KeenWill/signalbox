...
A `commission_session` request, and a pursuit-starting `attach_goal`, `resume_goal`,
or `supersede_goal` request for a pull-request-commissioned session, additionally
admits the transient rejection `commission_target_busy { session_id }` when
another live commissioned session already owns the same target. `session_id`
identifies that authoritative session so callers can wait and retry the exact
same command identity and payload after the competing session becomes terminal.
