...
`crates/persistence/migrations/` — seventy-four files, `202607180001` through
`202608100008` — embedded by `sqlx::migrate!` as the static `MIGRATOR` and
...
- Migration `202608100004` gives every new durable connection-loss epoch a
  ...
  workspace release.
- Migration `202608100005` gives every changed registration beyond revision one
...
- Migration `202608100006` distinguishes active and replacement-pending
...
`crates/persistence/src/lock_inventory.rs`. Twenty-five explicit lock
...
- the pending-successor registration and connection guards in migration
  `202608100006` take `FOR SHARE` or `FOR UPDATE`, respectively, on the
