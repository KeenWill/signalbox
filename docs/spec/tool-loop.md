...
`push_remote_url`; execution resolves that session's retained dispatch and
...
resolved commit before acknowledging success. For a two-parent merge, exactly
one parent must equal or descend from the retained-head fence; it is the branch
parent, regardless of parent order. A missing or ambiguous fence binding refuses
the push with `UnprovenMergeParents`. Multiple merge bases refuse it with
