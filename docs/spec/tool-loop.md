...
replacement text; non-conflicting text stays verbatim. Conflicting generated
Markdown marked in the base with `<!-- generated by ..., do not edit -->`, and
raw JSON fixture-literal hunks in Rust test paths, may be regenerated instead of
retaining the base lines verbatim; other test code remains subject to the line
check. A missing base effect or a changed non-conflicting span refuses the push
with `MergeDroppedBaseChanges` and a hunk preview. Rename detection correlates
parent renames through their common source paths, permitting either parent's
destination while retaining exact carried destinations. Non-text changes
relative to the base parent must occur in the branch diff. Collected
