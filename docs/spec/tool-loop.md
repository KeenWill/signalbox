...
replacement text; non-conflicting text stays verbatim. Generated Markdown marked
in the base with `<!-- generated by ..., do not edit -->` may be regenerated
across overlapping parent edits, including equal parent outputs. Classification
uses its base-parent path; adjacent edits remain non-conflicting, while
insertions at the same position overlap. Conflicting raw JSON fixture-literal
hunks in Rust test paths may also be regenerated; other test code remains
subject to the line check. A missing base effect or a changed non-conflicting
span refuses the push with `MergeDroppedBaseChanges` and a hunk preview. Rename
detection correlates parent renames through their common source paths,
