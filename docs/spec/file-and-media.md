...
| ---------------------- | ------------------------------------------------------------------------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Archives / compression | `application/zip`, `application/x-tar`, `application/gzip`, `application/zstd` | `entries` | `zip` 8 with stored and Deflate entries; other ZIP compression methods receive `unsupported_compression_method`; `tar` 0.4; `flate2` 1 with `zlib-rs`; and `zstd` 0.13, compiled only into `signalbox-file-media-archives-worker`; extraction, execution, link following, special TAR entries, recursive containers, password input, and interpretation of decompressed payloads are excluded |
...
yet on `main`. The archive and compression adapter is registered only in its
dedicated worker catalog, so the daemon's present empty registry still
recognizes no format. The compatibility constraint is that daemon composition
supplies the existing visibility proof to `FileUseResolver`, not a weaker
catalog-presence check. Format adapters add no MIME branch to the executor,
bridge, or daemon.
