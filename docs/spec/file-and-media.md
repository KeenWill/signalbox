...
(`agent/file-media-worker`; via PR #909 `agent/file-media-adapter-archives`).
...
| ---------------------- | ------------------------------------------------------------------------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Archives / compression | `application/zip`, `application/x-tar`, `application/gzip`, `application/zstd` | `entries` | `zip` 8 (stored/Deflate; otherwise `unsupported_compression_method`), `tar` 0.4, `flate2` 1 (`zlib-rs`), `zstd` 0.13 (external dictionaries produce `unsupported_dictionary`), only in `signalbox-file-media-archives-worker`; excludes extraction, execution, links, special TAR entries, recursion, passwords, external dictionaries, and payload interpretation |
