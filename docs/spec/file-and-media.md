...
The provider-neutral core is verified against PR #898 (`agent/file-media-core`). Its
isolated processor implementation is verified against PR #900
(`agent/file-media-worker`). Together they include the type model, declaration and
registry checks, detection and validation algorithm, untrusted processor-response
boundary, stable agent tool contracts, visibility-authorizing application bridge,
and fresh daemon-supervised worker runtime.
...
| ------ | --------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SVG    | `image/svg+xml` | `text`, `metadata` | `quick-xml` 0.41, compiled only into `signalbox-file-media-svg-worker`; rendering, scripts, event handlers, styles, DTDs, non-built-in entities, external or embedded resources, nested SVG containers, OCR, animation, and resource fetching are excluded |
...
yet on `main`. The SVG adapter is registered only in its dedicated worker
catalog, so the daemon's present empty registry still recognizes no format. The
compatibility constraint is that daemon composition supplies the existing
