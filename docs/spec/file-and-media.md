...
Together the provider-neutral core and isolated processor implementation include
the type model, declaration and registry checks, detection and validation
algorithm, untrusted processor-response boundary, stable agent tool contracts,
...
| Archives / compression | `application/zip`, `application/x-tar`, `application/gzip`, `application/zstd`                                                                                                                                              | Signature collection over a bounded 1 KiB prefix, then bounded complete-source structural arbitration for ZIP and for prefix-matched gzip/zstd within the 256 KiB source ceiling, so detection reads at most the prefix plus one remainder range; bounded container walk under 256 KiB source, 1,000-entry, 8 MiB per-entry, and 16 MiB expanded-byte ceilings | Bounded entry inventory as `entries`                                                  | `zip` 8 (stored/Deflate; otherwise `unsupported_compression_method`), `tar` 0.4, `flate2` 1 (`zlib-rs`), `zstd` 0.13 (frames declaring a nonzero external dictionary ID produce `unsupported_dictionary`; ID-less dictionary decode failures are malformed), only in `signalbox-file-media-archives-worker`; excludes extraction, execution, links, special TAR entries, recursion, passwords, external dictionaries, and payload interpretation |
| Video                  | `video/mp4`, `video/webm`                                                                                                                                                                                                   | Structural ISO BMFF and EBML detection over a bounded 4 KiB initial prefix, extended by at most four bounded 64 KiB follow-up ranges when the identifying brand or DocType sits past that prefix, so detection reads at most the 256 KiB metadata window; in-worker container parse confined to that same 256 KiB metadata prefix                              | Bounded track count, duration, and container brand as `metadata`                      | In-worker ISO BMFF and EBML parsing with no codec library; frame/audio decoding, rendering, frame extraction, thumbnails, OCR, transcription, subtitles, external resources, recursive containers, and metadata outside that prefix are excluded                                                                                                                                                                                                 |
