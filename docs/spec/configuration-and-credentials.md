...
- `signalbox_scheduler_passes_in_flight`, with no labels, reports current
  authoritative scheduler-pass occupancy: the count of passes holding admission
  slots.
- `signalbox_scheduler_oldest_in_flight_pass_age_seconds`, with no labels,
  reports the oldest admitted pass's age at scrape time, or zero while the loop
  is idle.
- `signalbox_scheduler_oldest_in_flight_pass_info{session_id}` reports the
  ...
  removes the former label value whenever the oldest pass changes or the loop
  becomes idle.
...
no turn id, model-call id, prompt, completion, or tool value. The durable
counters' source is the already-committed typed outbox transition, and
