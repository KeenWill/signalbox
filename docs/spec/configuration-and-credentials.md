...
make and code cannot observe which side of it a given operator is on. Two
profiles whose validated credential-home identities differ are independent token
families with nothing shared, so a pool holds as many as a deployment has and
runs them concurrently. The same lexical normalization used for `file` paths
