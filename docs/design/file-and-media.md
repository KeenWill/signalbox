...
chooses among repeated uses of the same bytes. Each rendered attachment stub
therefore carries that selector, the semantic entry's identity and the part's
zero-based ordinal within it, because the stub members named on
[blob storage](../spec/blob-storage.md) distinguish no two uses of one digest.
The resolver finishes all catalog work, then returns the exact file use and a
placement-free verified source, so no database transaction stays open into
source or worker I/O. A continuation request presents the cursor from the
preceding visible result. The cursor, or the state it authenticates, binds the
digest, selector, reader, view, and normalized options of the first read, so
later pages keep the first result's semantics, and the bound digest, selector,
reader, and view must still name a stub in the current allow-set. Once a stub
leaves the rendered frontier, a remembered digest, selector, or cursor grants
nothing.
...
reader that validated it, and its own content-silent evidence. A derived view
whose output reader differs from the source reader therefore keeps both. For
direct presentation both identities name the same digest, media type, and
reader, and the evidence is copied from the inspection. For a derived view the
...
the view, process, and target bounds. Derived bytes must fit those same target
bounds, and bytes that exceed them fail the read.
