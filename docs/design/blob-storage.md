...
with three committed capabilities.

## Goal

A `program_journal` storage class lets the workflows host store over-threshold
journal payloads in a routed store and reference them from the catalog.
Transcript projections carry blob descriptors and URLs so a browser can render
attachments from the transcript alone. A modality-unsupported preparation
failure closes a call whose rendered request carries media its target cannot
accept.
