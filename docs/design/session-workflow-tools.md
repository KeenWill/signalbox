...
Read computes input and result prefixes and extents against the existing 1 MiB
`ToolResultText` ceiling, including serialized response overhead, so a valid
read never fails as oversized. It includes registration identity and journal
length. Stop preserves the socket's `applied`, `not_found` and
...
workspace. Registration admits at most the same frame-bounded byte count as the
socket registration: the existing 8 MiB process-frame ceiling. Larger files
receive a typed refusal. Native registration is unavailable.
...
denied. Grants and operation postures come from the session's retained template
snapshot, bound to the same template name and content digest. They never come
from the live catalog: `reload_configuration` cannot change a live session's
grants. Each proposed call freezes its selected posture. Execution checks its
operation and target grant independently of approval and returns a retained
typed tool failure on refusal. Approval cannot widen a grant.
