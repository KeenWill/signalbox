...
- **Identity mechanics left open after ADR-0022.** [ADR-0022](decisions/0022-persistence-representation.md) selects native Postgres identity/reference columns only for the UUID-backed identities it names. The [2026-07-17 context-frontier representation decision](decisions.md#2026-07-17--uuid-backed-context-frontier-values-and-sealed-prefix-derivation) separately selects private UUID-backed in-process `ContextFrontierId` and `SemanticTranscriptEntryId` values. UUID generation and version, caller supply versus hub minting, boundary validation, public formatting, serialization, wire encoding, identity encodings embedded in canonical payloads, and the database encoding of those newly backed identities remain open. Blocks protocol and identity-minting slices. (S01, S02, S04, S08, S10, S12)
