...
- **Provider headroom observation.** Selecting a profile by remaining capacity
  ...
  rule in
  [credential pools and selection](spec/configuration-and-credentials.md#decisions),
...
- **Zero-cost liveness probes.** Quarantine semantics are decided and owned by
  [credential pools and selection](spec/configuration-and-credentials.md#contracts):
...
- **Access-token-only Codex CLI conformance evidence.** The committed `oauth`
  ...
  [credential deliveries](spec/configuration-and-credentials.md#not-built). What
...
- **Several bound workspaces per session, and explicit session relocation.** A
  ...
  [configuration and credentials](spec/configuration-and-credentials.md#map),
...
- **Runner result credential egress beyond exact-value redaction.** Whether
  ...
  [runner credential boundary](spec/configuration-and-credentials.md#not-built)
