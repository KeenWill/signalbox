...
## Repository-watch store replacement

Owner decision pending: retain the module store or replace it with workflow
state and host receipts? Path A, the committed
[repository watch design](design/repo-watch.md), retains module storage and
moves orchestration into workflows. Path B is an alternative for owner
consideration: remove `mod_repo_watch` and make journaled workflow state plus
host-owned receipts authoritative. The comparison does not select B or amend the
host contract. Both paths retain the provider client, pure differ/matchers,
webhook listener, credential boundaries, checkout operations and separate
convergence sweep.
...
A uses the flag handoff in [repository watch design](design/repo-watch.md). B
can start fresh on an empty installation; otherwise it needs B-readable accepted
events, revision history, sticky suppression and session origins before dropping
their module records. Draining pending intake, live dispatches and cleanup
simplifies transfer but does not erase that retained history. An undrained
cutover also transfers pending commands and checkout/retirement evidence. Empty
initial databases do not erase work accepted during coexistence. B cannot
promise A's immediate rollback while also dropping the store and forbidding
state transfer; schema removal follows owner acceptance of that cutover
constraint.
