...
The intended publication mechanism inside the hub is a **transactional outbox**:
each durable transition records its outbound update event in the same Postgres
transaction that commits the transition, and a separate publisher delivers
events to subscriptions afterward, so a crash between commit and delivery loses
no committed transition and redelivery is absorbed by cursor ordering. Accepted
records leave this mechanism open
([ADR-0022](decisions/0022-persistence-representation.md) lists outbox and
delivery mechanics among its open questions); a decision for it has been
proposed as ADR-0040 (transactional outbox) and, until accepted, the outbox
remains a target.
