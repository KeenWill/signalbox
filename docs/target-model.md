...
that the spec does not yet describe are **targets awaiting decisions, not
decisions**: naming a concept here authorizes neither its implementation nor a
silent closure of an open question. The [one-place rule](../AGENTS.md) applies
throughout — implemented semantics are linked, never restated, and only
target-only concepts are owned by this document.
