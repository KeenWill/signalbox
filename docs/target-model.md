...
`docs/spec/` and the generated APIs for the
[configured crates](api/crates.toml); where this document and those pages
disagree about what the system does, they win and this document describes only
where the system is headed. The [glossary](glossary.md) is a terminology index,
not part of that normative surface. Concepts described here that the spec does
not yet describe are **targets awaiting decisions, not decisions**: naming a
concept here authorizes neither its implementation nor a silent closure of an
