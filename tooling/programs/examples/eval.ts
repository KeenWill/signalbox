...
const output = jsonCodec((value: unknown): { verdicts: string[] } => {
  ...
  for (let index = 0; index < value.verdicts.length; index++) {
    if (typeof value.verdicts[index] !== "string") throw new TypeError("expected trial outcomes");
  }
  return { verdicts: value.verdicts as string[] };
});
...
export default defineProgram({
  ...
  async run(input) {
    ...
    if (blob.kind !== "answer") throw new Error("blob unavailable");
    const corpus = await evaluation.corpus();
    if (corpus.kind !== "answer") throw new Error("corpus unavailable");
    const verdicts: string[] = [];
    for (let trial = 0; trial < input.cases.length * input.repeats; trial++) {
      const result = await evaluation.judge({ trial });
      if (result.kind !== "answer") throw new Error("judge unavailable");
