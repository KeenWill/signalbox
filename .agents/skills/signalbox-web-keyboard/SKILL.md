...
Yield a key only when it natively activates the focused control: for example,
yield `Enter` for links and `Enter` or `Space` for buttons. When that control
invokes a registered command, its native activation is the single execution
path; the application keymap must not dispatch it a second time. Other
key/control pairs remain available to the application keymap.
