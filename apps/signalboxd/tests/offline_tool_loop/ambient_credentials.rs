...
impl ProcessRunner for AmbientRunner {
    ...
    async fn bwrap_availability(&mut self, probe: ProcessRequest) -> BwrapAvailability {
        assert_eq!(probe.program, "bwrap");
        BwrapAvailability::Available
    }

    async fn run(&mut self, request: ProcessRequest) -> ProcessRunResult {
        assert_eq!(request.program, "bwrap");
        assert!(request.arguments.contains(&"--unshare-net".into()));
        assert!(!request.environment.contains_key(OsStr::new(VARIABLE)));
        let mount = request.arguments.windows(3).find(|args| {
            args[0] == "--ro-bind" && args[2] == self.destination.as_os_str()
        });
        let environment = request.arguments.windows(3).find(|args| {
            args[0] == "--setenv" && args[1] == VARIABLE
        });
        ...
        if !secret.is_empty() {
            ...
            ).fetch_one(&self.pool).await.expect("judge provenance recorded before execution");
            assert!(approved);
        }
        ...
        ProcessRunResult {
            ...
            stdout: ProcessOutput { bytes: secret.clone(), completeness: CaptureCompleteness::Complete },
            stderr: ProcessOutput { bytes: secret, completeness: CaptureCompleteness::Complete },
        }
    }
}
...
async fn judge_gates_each_ambient_task_and_credentials_end_with_the_task() -> Result<(), Box<dyn Error>> {
    ...
    for delivery in ["file", "variable"] {
        for recommendation in ["approve", "deny"] {
            ...
            let source = if delivery == "file" { credential.path().to_str().expect("path") } else { VARIABLE };
            let models = HubModelConfiguration::parse(&format!(
                "{}\n[[credential_profiles]]\nname = {PURPOSE:?}\nadapter = \"sandboxed_exec\"\ndelivery = \"ambient\"\n{delivery} = {source:?}\n",
                approval_judge_model_configuration().source()
            ))?;
            let catalogs = signalboxd::configuration_reload::ConfigurationReload::new(
                fixture.pool.clone(), models.clone(), Default::default(), workspace.path().join("models.toml"), workspace.path().join("templates.toml"), None,
            ).expect("catalogs");
            let runner = AmbientRunner { requests: Default::default(), pool: fixture.pool.clone(), destination: credential.path().to_owned() };
            let (catalog, executor) = DaemonTools::try_new(
                (|| SystemTime::UNIX_EPOCH) as fn() -> SystemTime,
                OfflineWebTransport::unused(),
                MappedDaemonCredentialInputs { web_search: OfflineCodeHostCredentials, code_host: OfflineCodeHostCredentials, github: OfflineCodeHostCredentials },
                UnusedWebSearchTransport, UnusedSessionStatusWriter, UnusedCodeHostTransport, UnusedGitHubTransport,
                GitHubEgressPolicy::github_api_only(), LocalWorkspaceFileSystem, workspace.path(), git_identity(), runner.clone(),
                PostgresConversationIntrospection::new(fixture.pool.clone()), signalbox_persistence::plan::SessionPlanRepository::new(fixture.pool.clone()), WebFetchEgressPolicy::deny_all(),
            )?.into_parts();
            let executor = executor.with_ambient_credentials(catalogs, fixture.pool.clone());
            let arguments = serde_json::json!({"program":"fixture-command","credential_purpose":PURPOSE}).to_string();
            let (execution, runtime, judge) = fixture.execution_with_judge_configuration(
                [tool_use_script(&[(SANDBOXED_EXEC_NAME, &arguments)]), tool_use_script(&[(SANDBOXED_EXEC_NAME, r#"{"program":"fixture-command"}"#)]), completion_script("tasks observed")],
                approval_judge_script(recommendation, "The exact credential purpose was evaluated."), catalog, executor, models,
            );
            execution.execute(Box::new(fixture.activated.clone())).await?;
            let requests = runner.requests.lock().expect("requests");
            assert_eq!(requests.len(), if recommendation == "approve" { 2 } else { 1 });
            let ordinary = requests.last().expect("ordinary task executes");
            assert!(!ordinary.arguments.iter().any(|value| value == credential.path().as_os_str() || value == VARIABLE));
            for request in requests.iter() {
                if let Some(mount) = request.arguments.windows(3).find(|args| args[0] == "--ro-bind" && args[2] == credential.path().as_os_str()) {
                    assert!(!std::path::Path::new(&mount[1]).exists(), "task snapshot removed");
                }
            }
            drop(requests);
            assert_eq!(judge.received_operations().len(), 1, "the blanket must still consult the judge");
