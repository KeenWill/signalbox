...
use signalbox_model_runtime::{
    ...
    CredentialReference, CredentialValue, ModelOperation, ModelRuntime, ModelSettings, Observation,
    ObservationSink, PreparationOutcome, RequestedTarget, ResolvedTarget,
};
...
fn mapped_daemon_catalog(workspace: &Path) -> DaemonToolCatalog {
    ...
    let status = SessionStatusTool::try_new(OfflineWriter).expect("offline status tool compiles");
    ...
    DaemonTools::try_new_with_tools(
        ...
        ComposedToolFamilies {
            ...
            plan,
            delegation: SessionDelegationTools::try_new(DaemonSessionDelegationPort::unavailable())
...
async fn capture_prepared_claude_catalog(
    ...
    let runtime =
        ClaudeCliRuntime::new(config).expect("offline Claude catalog capture runtime constructs");
...
fn captured_catalog_path_uses_semantic_mcp_configuration() {
    ...
    assert_eq!(
        claude_catalog_paths_from_config(config.as_bytes(), Path::new(SYNTHETIC_MCP_BRIDGE_PATH),),
        Some(CapturedClaudeMcpPaths {
...
const SYNTHETIC_GOAL_DATABASE_URL: &str = "postgresql://signalbox:synthetic@127.0.0.1/signalbox";
...
fn admitted_configured_target_dir(input: AdmittedConfiguredTargetDirInput<'_>) -> Option<PathBuf> {
...
fn rustc_target_names(invocation_directory: &Path) -> BTreeSet<OsString> {
    let rustc =
        normalized_rustc_override(invocation_directory).unwrap_or_else(|| PathBuf::from("rustc"));
...
fn rustc_command_for(configured: Option<&OsStr>, invocation_directory: Option<&Path>) -> PathBuf {
...
fn relative_target_dir_uses_the_captured_cargo_change_directory() {
    ...
    let invocation = cargo_test_invocation_from_arguments(&arguments, invocation_directory.path())
...
fn bridge_artifact_selection_rejects_a_custom_target_reusing_a_builtin_name() {
    let custom_target =
        Path::new("synthetic-target-specifications").join(format!("{SYNTHETIC_CARGO_TARGET}.json"));
    let arguments = [
        OsStr::new(CARGO_PROGRAM_STEM),
        OsStr::new(CARGO_TEST_SUBCOMMAND),
...
fn bridge_artifact_selection_canonicalizes_a_symlinked_target_directory() {
    ...
    std::os::unix::fs::symlink(&target_dir, &target_link).expect("fixture target symlink exists");
...
fn bridge_artifact_selection_recovers_a_differently_named_relative_symlink_root() {
    ...
    std::os::unix::fs::symlink(&target_dir, &target_link).expect("fixture target symlink exists");
    let executable = target_dir.join("debug/deps/daemon-tools-test");
    let configured_target_dir = configured_cargo_target_dir_for(ConfiguredCargoTargetDirInput {
        current_executable: &executable,
...
fn bridge_build_rejects_an_ambiguous_directly_invoked_test_binary() {
    ...
    assert!(
        stdout.contains("direct Cargo test artifacts must retain an unambiguous profile directory")
    );
}
...
fn cargo_bridge_executable(messages: Receiver<Result<String, io::Error>>) -> PathBuf {
    messages
        ...
        .map(|message| {
            serde_json::from_str::<serde_json::Value>(&message).expect("Cargo build output is JSON")
        })
...
fn composed_catalog_applies_an_enforceable_posture() {
    let (echo_catalog, _executor) = EchoTool::try_new()
        ...
        .into_parts();
    let catalog =
        DaemonToolCatalog::try_new([echo_catalog]).expect("single-tool fixture has unique names");
...
fn daemon_catalog_without_mappings_contains_only_base_families() {
    ...
    let (catalog, _executor) = DaemonTools::try_new_with_tools(
        ...
        ComposedToolFamilies {
            ...
            plan: PlanTools::try_new(OfflineConversationPort).expect("offline plan tools compile"),
            delegation: SessionDelegationTools::try_new(DaemonSessionDelegationPort::unavailable())
                .expect("offline session-delegation tools compile"),
            goal: None,
        },
...
impl McpBridgeFixture {
    ...
    fn start_with_workspace_and_definitions(
        ...
        let catalog_path = support.path().join(MCP_CATALOG_FILENAME);
        let ready_path = support.path().join(MCP_READY_FILENAME);
        fs::write(&catalog_path, &projected_catalog.catalog).expect("bridge catalog is written");
    ...
    fn list_tools(&mut self) -> Vec<ComparableBridgeTool> {
        ...
        let response: ListedBridgeResponse =
            serde_json::from_str(&response).expect("MCP list response preserves raw tool schemas");
        response
...
async fn a_replaced_administration_directory_fails_the_next_request() {
    ...
    let after_replacement = daemon_evidence(catalog, executor, first, read_marker_proposal()).await;
...
fn two_pinned_filesystems_answer_for_their_own_roots() {
    ...
    let first =
        PinnedWorkspaceFileSystem::pin_further_root(&first_root).expect("the first root is pinned");
...
fn idle_overflow_drains_back_to_the_bound() {
    ...
    const IDLE_MARKER: u32 = 2;
    let mut retained = RetainedSessionWorkspaces::new();
    retain_in_flight_over_the_bound(&mut retained, RetainedFixture::in_flight(IN_FLIGHT_MARKER));
