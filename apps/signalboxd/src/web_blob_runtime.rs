...
// numeric-bound: guard - prevents one declared image dimension from driving an unbounded decoder
// allocation
...
// numeric-bound: guard - prevents an encoded derivative from exhausting process memory when it is
// read back
const MAX_DERIVATIVE_BYTES: u64 = 16 * 1024 * 1024;
// numeric-bound: guard - prevents an oversized source blob from being copied into the worker's
// input
...
// numeric-bound: guard - prevents a wedged derivative worker from holding its derivation slot
// forever
