...
impl RepositoryWatchTask {
    ...
    async fn resolve_targeted_webhook_observation(
            ...
            .await?
        {
            TargetedPollOutcome::Observation {
                observation,
                convergence,
                superseded_targets: _,
            } => Ok(TargetedPolledRepository::Refreshed {
                observation,
                convergence,
            }),
            TargetedPollOutcome::SupersededTarget => Ok(TargetedPolledRepository::Superseded),
        }
    }
    ...
    async fn prepare_targeted_refresh(
        ...
        let mut event_identity_frontier = cursor.candidate().event_identity_frontier().clone();
        let (observation, convergence, superseded_targets) = match self
            .poller
            ...
            .await?
        {
            TargetedPollOutcome::Observation {
                ...
                convergence,
                superseded_targets,
            } => (observation, convergence, superseded_targets),
