...
// numeric-bound: guard - prevents a wedged S3 namespace-prepare or multipart-lifecycle probe from
// blocking daemon startup forever
const S3_STARTUP_DEADLINE: std::time::Duration = std::time::Duration::from_secs(5 * 60);
// numeric-bound: guard - prevents unbounded concurrent blob reads from exhausting process memory
// and store handles
