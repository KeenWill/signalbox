...
def load_config():
    ...
    config["executor"] = config.get("executor") or os.environ.get("BENCHMARK_EXECUTOR") or "local"
    return config
...
def main():
    ...
    if executor not in ("local", "rbe"):
        raise ValueError("executor must be local or rbe")
    mode = config["mode"]
    ...
    for repetition in range(1, runs + 1):
        ...
        if executor == "rbe":
            flags.extend(["--config=rbe", f"--remote_cache={endpoint}"])
        if mode == "cold":
            flags.append("--noremote_accept_cached")
        command = [*bazel, "test", *flags, "--", *targets]
        metadata = {
            "label": config["label"], "mode": mode, "executor": executor,
