...
class SummaryTests(unittest.TestCase):
    def test_executor_selection_preserves_local_defaults_and_pr_override(self):
        for executor in ("local", "rbe"):
            config = {"executor": executor, "cache_endpoint": "grpc://cache.invalid"}
            with self.subTest(executor=executor), patch.dict(os.environ, {
                "GITHUB_EVENT_NAME": "pull_request", "BENCHMARK_EXECUTOR": "local",
            }), patch.object(Path, "read_text", return_value=json.dumps(config)):
                self.assertEqual(load_config()["executor"], executor)
        with patch.dict(os.environ, {
            "GITHUB_EVENT_NAME": "workflow_dispatch", "CACHE_ENDPOINT": "grpc://cache.invalid",
            "BENCHMARK_MODE": "warm", "BENCHMARK_RUNS": "1", "BENCHMARK_TARGETS": "//:rust_build",
            "BENCHMARK_LABEL": "dispatch", "BENCHMARK_EXECUTOR": "rbe",
        }):
            self.assertEqual(load_config()["executor"], "rbe")

    def test_remote_configuration_follows_local_resource_flags(self):
        for executor in ("local", "rbe"):
            commands = []
            def run(command, **kwargs):
                if "help" in command:
                    kwargs["stdout"].write("build_event_json_file execution_log_compact_file remote_grpc_log profile remote_accept_cached")
                elif "test" in command:
                    commands.append(command)
                return subprocess.CompletedProcess(command, 0)
            with self.subTest(executor=executor), tempfile.TemporaryDirectory() as temp:
                config = dict(cache_endpoint="grpc://cache.invalid", mode="cold", runs=1,
                              targets="//:rust_build", label="executor", executor=executor)
            ...
            command, = commands
            self.assertEqual("--config=rbe" in command, executor == "rbe")
            if executor == "rbe":
