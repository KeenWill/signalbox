...
struct SessionDetailScreen: View {
    ...
    @State private var selectedArtifact: SignalboxArtifact?

    init(session: SignalboxSessionMetadata) {
                    ...
                    LazyVStack(alignment: .leading, spacing: 12) {
                        ...
                        if viewModel.timelineItems.isEmpty {
                            EmptyStateView(
                                ...
                                title: "No events yet",
                                message: "Send a message to start a turn on the server."
                            )
                        }
                    }
                    .padding()
    ...
    @ViewBuilder
    private func timelineView(for item: SignalboxTimelineItem) -> some View {
        switch item {
...
private struct PromptContextArtifactCard: View {
    let artifact: SignalboxArtifact
    let onOpen: () -> Void
