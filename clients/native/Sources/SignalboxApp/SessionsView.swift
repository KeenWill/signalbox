...
struct SessionsScreen: View {
    ...
    @State private var selectedSessionID: SignalboxSessionID?
            ...
            VStack(spacing: 0) {
                ...
                if viewModel.visibleSessions.isEmpty && !viewModel.isLoading {
                    EmptyStateView(
                        ...
                        message: "Create a session from a template or refresh after connecting to the server."
                    )
                } else {
                    List(viewModel.visibleSessions) { session in
                        Button {
                            ...
                            SessionRowView(
                                session: session,
                                status: viewModel.statusesBySessionID[session.id] ?? SignalboxSessionStatus(state: .idle)
                            )
                        }
                        .buttonStyle(.plain)
...
private struct SessionRowView: View {
    ...
    let status: SignalboxSessionStatus
...
struct CreateSessionSheet: View {
    ...
    let onCreated: (SignalboxSessionMetadata) -> Void

    @Environment(\.dismiss) private var dismiss
    @State private var selectedTemplateID: SignalboxTemplateID?
    @State private var selectedRunnerID: SignalboxRunnerID?
            ...
            Form {
                Section {
                    ...
                    Picker("Template", selection: $selectedTemplateID) {
                        Text("Explicit prompt").tag(Optional<SignalboxTemplateID>.none)
                    ...
                    Picker("Runner", selection: $selectedRunnerID) {
                        Text("Auto placement").tag(Optional<SignalboxRunnerID>.none)
                ...
                if let template = selectedTemplate {
                    Section("Template Details") {
                        NewSessionTemplateDetails(
                            ...
                            model: template.defaultModelAlias ?? "Server default",
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    NewSessionFormField(label: "Workspace Path") {
                        TextField("projects/signalbox", text: $linkedWorkspacePath)
                            .textFieldStyle(.roundedBorder)
                            .accessibilityIdentifier("linked-workspace-path")
                    }
    ...
    private var selectedTemplate: SignalboxTemplate? {
