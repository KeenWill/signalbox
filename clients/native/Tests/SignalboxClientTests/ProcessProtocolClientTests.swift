...
final class ProcessProtocolClientTests: XCTestCase {
  func testMalformedKnownMessageDoesNotKillFollowingFrame() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          """
          {"version":21,"request_id":"1","message":{"type":"session_created","session_id":17}}
          {"version":21,"request_id":"1","message":
          """.utf8
        ),
        Data(
          ("""
  ...
  func testClientWritesOneNewlineTerminatedVersionTwentyOneFrame() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          ("""
          {"version":21,"request_id":"1","message":{"type":"sessions_end","session_count":"0"}}
          """ + "\n").utf8
        )
      ]
    )
    ...
    XCTAssertEqual(
      ...
      #"{"request":{"type":"list_sessions"},"request_id":"1","version":21}"# + "\n"
    )
  }
  ...
  func testPullExchangeReadsNoFollowingChunkBeforeNextRequest() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          ("""
          {"version":21,"request_id":"1","message":{"type":"sessions_start"}}
          """ + "\n").utf8
        ),
        Data(
          ("""
  ...
  func testCancellationBeforeSendNeverWritesRequest() async {
    let connection = ScriptedProcessConnection(chunks: [], suspendsStart: true)
    let client = SignalboxProcessClient(
      connectionFactory: ScriptedProcessConnectionFactory(connection: connection)
    )
    let opening = Task { await capturedCancellation(client) }
    await connection.waitUntilStartStarted()

    opening.cancel()
    await connection.completeStart()
  ...
  private func capturedCancellation(
    _ client: SignalboxProcessClient
  ) async -> Bool {
    do {
      _ = try await client.open(.listSessions)
...
private actor ScriptedProcessConnection: SignalboxProcessConnection {
  ...
  private var startStartedContinuation: CheckedContinuation<Void, Never>?
  ...
  func start() async throws {
    ...
    if suspendsStart {
      await withCheckedContinuation { continuation in
        ...
        startStartedContinuation?.resume()
        startStartedContinuation = nil
      }
    }
  }
  ...
  func waitUntilStartStarted() async {
    ...
    await withCheckedContinuation { continuation in
      startStartedContinuation = continuation
    }
  }
