...
final class ProcessProtocolClientTests: XCTestCase {
  func testMalformedKnownMessageDoesNotKillFollowingFrame() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          """
          {"version":1,"request_id":"1","message":{"type":"session_created","session_id":17}}
  ...
  func testClientWritesOneNewlineTerminatedVersionOneFrame() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          ("""
          {"version":1,"request_id":"1","message":{"type":"sessions_end","session_count":"0"}}
          """ + "\n").utf8
        )
      ]
    )
    ...
    XCTAssertEqual(
      ...
      #"{"request":{"type":"list_sessions"},"request_id":"1","version":1}"# + "\n"
    )
  }
  ...
  func testPullExchangeReadsNoFollowingChunkBeforeNextRequest() async throws {
    let connection = ScriptedProcessConnection(
      chunks: [
        Data(
          ("""
          {"version":1,"request_id":"1","message":{"type":"sessions_start"}}
          """ + "\n").utf8
        ),
        Data(
          ("""
