...
TEST_IDENTIFIER="SignalboxNativeUITests/SignalboxNativeUITests/testRealServerConnectionListsRunnerAndCreatesSessionWhenConfigured"
XCTESTRUN_TEST_KEY="${SIGNALBOX_NATIVE_XCTESTRUN_TEST_KEY:-}"

export SIGNALBOX_NATIVE_REAL_SERVER_URL="${SIGNALBOX_NATIVE_REAL_SERVER_URL:-http://127.0.0.1:8000}"

if [[ -z "${SIGNALBOX_NATIVE_REAL_SERVER_API_KEY:-}" ]]; then
	echo "Set SIGNALBOX_NATIVE_REAL_SERVER_API_KEY to the server API key value before running the real server smoke test." >&2
	exit 2
fi
...
restore_real_server_env_file() {
	if [[ -z "$REAL_SERVER_ENV_BACKUP" ]]; then
		return 0
	fi

	if ((REAL_SERVER_ENV_EXISTED)); then
		cp "$REAL_SERVER_ENV_BACKUP" "$REAL_SERVER_ENV_PATH"
	else
		rm -f "$REAL_SERVER_ENV_PATH"
	fi
	rm -f "$REAL_SERVER_ENV_BACKUP"
}

REAL_SERVER_ENV_BACKUP="$(mktemp "${TMPDIR:-/tmp}/signalbox-native-real-server-env.XXXXXX")"
if [[ -f "$REAL_SERVER_ENV_PATH" ]]; then
	cp "$REAL_SERVER_ENV_PATH" "$REAL_SERVER_ENV_BACKUP"
	REAL_SERVER_ENV_EXISTED=1
fi
trap restore_real_server_env_file EXIT
mkdir -p "$(dirname "$REAL_SERVER_ENV_PATH")"
rm -f "$REAL_SERVER_ENV_PATH"
touch "$REAL_SERVER_ENV_PATH"
...
XCTESTRUN_PATH="${SIGNALBOX_NATIVE_XCTESTRUN_PATH:-}"
...
xctestrun_key_matches_ui_tests() {
	...
	[[ "$blueprint_name" == *SignalboxNativeUITests* || "$test_bundle_path" == *SignalboxNativeUITests.xctest* ]]
}
...
if ! XCTESTRUN_TEST_TARGET_PATH="$(resolve_xctestrun_test_target_path)"; then
	echo "Could not find the SignalboxNativeUITests entry in $XCTESTRUN_PATH." >&2
	echo "Top-level .xctestrun keys:" >&2
	list_xctestrun_top_level_keys >&2
	echo "Set SIGNALBOX_NATIVE_XCTESTRUN_TEST_KEY to one of those keys if needed." >&2
	exit 65
fi
