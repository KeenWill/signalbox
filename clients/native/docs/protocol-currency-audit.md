...
- read-only `list`, transcript, and follow observations from existing live
  sessions, including a continuation materialized from an imported conversation
  and a multi-page imported catalog. No live mutation was sent.

The live transcript samples exercised completed turns, user and assistant text,
terminal model-call usage records, completion markers, and ordered imported
source-event, thinking, tool-call, and tool-result markers. Follow confirmed
that the same durable conversation can be reconstructed through the streaming
read surface. The multi-page catalog observation confirmed that refresh eagerly
reads sequential pages. Live observation is evidence of exercised shapes, not
evidence that unobserved protocol variants are absent.
...
14 close-now, 12 staged, and 3 report-only.
...
| S11 | High     | `listConversations` eagerly reads every 100-row page on each refresh and throws when its 100-page application cap is reached instead of publishing a bounded prefix.                                                                                                                                                                                                                                                                                                                                                                                                                                       | **staged** — design incremental catalog loading and an explicit end-of-retention presentation. |
| S12 | Medium   | Imported detail reads retain and project the entire transcript before publishing it; the application cap permits 50,000 entries, so latency and memory grow linearly with conversation size.                                                                                                                                                                                                                                                                                                                                                                                                               | **staged** — design incremental or paged detail projection without weakening ordering checks.  |
...
| --- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| S08 | Medium   | The template catalog exposes only template name and bundle version, which is insufficient for a useful native browser without another source of display metadata. | **staged** — define public template summary metadata before templates IA.                       |
| S09 | Medium   | The current process protocol exposes no independent goal-mode read model for a native goal-mode display.                                                          | **staged** — establish a read contract before designing goal-mode UI.                           |
| S10 | Medium   | Plans are available only as model tool calls/results; there is no independent client plan-read verb or server projection.                                         | **staged** — C11/C12 improve transcript fidelity without inventing a plan API.                  |
| R02 | High     | Import rejection collapses 17 observed failure classes to one opaque `invalid_request` response and emits no diagnostic log, erasing actionable failure detail.   | **report-only** — add typed or logged daemon diagnostics in a daemon-owned change.              |
| R03 | High     | The daemon's 6 MiB import-frame ceiling rejected 70.8% by volume of one observed Codex-rollout corpus before schema processing.                                   | **report-only** — revisit transport/import framing and size admission in a daemon-owned change. |
