...
for (const view of ['preview', 'thumbnail'] as const) {
  test(`renders tool raster results through their ${view} route`, async ({ page }, testInfo) => {
    ...
    await row.getByRole('button', { name: 'Open Image details', exact: true }).focus()
    ...
    await media.getByRole('img', { name: label }).scrollIntoViewIfNeeded()
    await expect(media.getByRole('img', { name: label })).toBeVisible()
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
    await page.screenshot({ path: testInfo.outputPath(`tool-${view}-phone.png`), fullPage: false })
