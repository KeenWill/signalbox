...
// Shell tests reach Attention, which now reads a live projection. Serve an empty
// bounded page and one snapshot event so no shell test depends on a running
// daemon; `product-attention.spec.ts` owns the populated scenarios.
const useDeterministicBootstrap = async (page: Page) => {
  await page.route('**/api/bootstrap', (route) => route.fulfill({ json: bootstrapFixture }))
  await page.route('**/api/attention**', (route) => {
    if (new URL(route.request().url()).pathname.endsWith('/follow')) {
